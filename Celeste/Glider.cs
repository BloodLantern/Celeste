    ...
    public class Glider : Actor
    {
        ...
        public override void Update()
        {
            ...
            if (Hold.IsHeld && !Hold.Holder.OnGround() && (sprite.CurrentAnimationID == "fall" || sprite.CurrentAnimationID == "fallLoop"))
            {
                ...
                fallingSfx.Param("glider_speed", Calc.Map(new Vector2(speed.X * 0.5f, speed.Y < 0.0 ? speed.Y * 2f : speed.Y).Length(), 0.0f, 120f, newMax: 0.7f));
            }
            else
                fallingSfx.Stop();
            base.Update();
            if (!destroyed)
            {
                ...
                else if (!bubble)
                {
                    ...
                    Rectangle bounds;
                    if (Left < (double)level.Bounds.Left)
                    {
                        bounds = level.Bounds;
                        Left = bounds.Left;
        ...
        private Vector2 PlatformAdd(int num) => new Vector2(num - 12, (int)Math.Round(Math.Sin(Scene.TimeActive + num * 0.20000000298023224) * 1.7999999523162842) - 5);
