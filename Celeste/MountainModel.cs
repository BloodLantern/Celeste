    ...
    public class MountainModel : IDisposable
    {
        ...
        public void BeforeRender(Scene scene)
        {
            ...
            if (StarEase > 0.0)
            {
                ...
                Matrix matrix5 = Matrix.CreateScale(0.25f) * Matrix.CreateRotationZ((float) Math.Cos(num * 2.0) * 0.5f) * Matrix.CreateRotationX((float) (0.40000000596046448 + Math.Sin(num) * 0.05000000074505806)) * Matrix.CreateRotationY((float) (-(double) num - 1.5707963705062866)) * Matrix.CreateTranslation((float) Math.Cos(num) * 2.2f, (float) (31.0 + Math.Sin(num * 2.0) * 0.800000011920929), (float) Math.Sin(num) * 2.2f);
            ...
            GaussianBlur.Blur((RenderTarget2D) buffer, blurA, blurB, 0.75f, samples: GaussianBlur.Samples.Five);
        }
        ...
        public void Render()
        {
            ...
            Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, null, null);
            Draw.SpriteBatch.Draw((RenderTarget2D) buffer, Vector2.Zero, buffer.Bounds, Color.White * 1f, 0.0f, Vector2.Zero, scale, SpriteEffects.None, 0.0f);
            Draw.SpriteBatch.Draw((RenderTarget2D) blurB, Vector2.Zero, blurB.Bounds, Color.White, 0.0f, Vector2.Zero, scale * 2f, SpriteEffects.None, 0.0f);
            Draw.SpriteBatch.End();
        }
    }
}
