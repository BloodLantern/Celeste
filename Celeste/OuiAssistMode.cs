...
namespace Celeste
{
    public class OuiAssistMode : Oui
    {
        ...
        private float fade;
        private readonly List<OuiAssistMode.Page> pages = new();
        ...
        public override IEnumerator Enter(Oui from)
        {
            ...
            if (!ouiAssistMode.FileSlot.AssistModeEnabled)
            {
                for (int index = 0; Dialog.Has("ASSIST_MODE_" + index); ++index)
                {
                    ...
                    {
                        Text = FancyText.Parse(Dialog.Get("ASSIST_MODE_" + index), 2000, -1, defaultColor: new Color?(Color.White * 0.9f)),
                        Ease = 0.0f
                    });
                }
        ...
        public override IEnumerator Leave(Oui next)
        {
            ...
            {
                _ = (int)ouiAssistMode.mainSfx.release();
            }
            ...
            while (!Input.MenuCancel.Pressed)
            {
                ...
                if (ouiAssistMode.pageIndex >= ouiAssistMode.pages.Count)
                {
                    if (Input.MenuConfirm.Pressed)
                    {
                        ...
                        ouiAssistMode.Focused = false;
                        _ = ouiAssistMode.Overworld.Goto<OuiFileSelect>();
                        _ = Audio.Play(ouiAssistMode.questionIndex == 0 ? "event:/ui/main/assist_button_yes" : "event:/ui/main/assist_button_no");
        ...
        public override void Render()
        {
            ...
            {
                OuiAssistMode.Page page = pages[index];
                float num = Ease.CubeOut(page.Ease);
                if ((double)num > 0.0)
                {
                    Vector2 position = new(960f, 620f);
                    position.X += (float)(page.Direction * (1.0 - (double)num) * 256.0);
            ...
            if (questionEase > 0.0)
            {
                ...
                Vector2 position = new((float)(960.0 + ((1.0 - (double)num1) * 256.0)), 620f);
                ...
                ActiveFont.DrawOutline(Dialog.Clean("ASSIST_YES"), position + new Vector2((float)((questionIndex == 0 ? (double)num2 : 0.0) * 1.2000000476837158) * num1, (float)(((double)lineHeight * 1.3999999761581421) + 10.0)), new Vector2(0.5f, 0.0f), Vector2.One * 0.8f, SelectionColor(questionIndex == 0), 2f, Color.Black * num1 * fade);
                ActiveFont.DrawOutline(Dialog.Clean("ASSIST_NO"), position + new Vector2((float)((questionIndex == 1 ? (double)num2 : 0.0) * 1.2000000476837158) * num1, (float)(((double)lineHeight * 2.2000000476837158) + 20.0)), new Vector2(0.5f, 0.0f), Vector2.One * 0.8f, SelectionColor(questionIndex == 1), 2f, Color.Black * num1 * fade);
            }
            if (pages.Count > 0)
            {
                ...
                Vector2 vector2 = new(960f, (float)(960.0 - (40.0 * (double)Ease.CubeOut(fade))));
                for (int index = 0; index < num3; ++index)
                {
                    mtexture.DrawCentered(vector2 + new Vector2((-num4 / 2) + (mtexture.Width * (index + 0.5f)), 0.0f), Color.White * 0.25f);
                }

                float x = (float)(1.0 + ((double)Calc.YoYo(dot % 1f) * 4.0));
                mtexture.DrawCentered(vector2 + new Vector2((-num4 / 2) + (mtexture.Width * (dot + 0.5f)), 0.0f), iconColor, new Vector2(x, 1f));
                GFX.Gui["dotarrow"].DrawCentered(vector2 + new Vector2((-num4 / 2) - 50, (float)(32.0 * (1.0 - (double)Ease.CubeOut(leftArrowEase)))), iconColor * leftArrowEase, new Vector2(-1f, 1f));
                GFX.Gui["dotarrow"].DrawCentered(vector2 + new Vector2((num4 / 2) + 50, (float)(32.0 * (1.0 - (double)Ease.CubeOut(rightArrowEase)))), iconColor * rightArrowEase);
            }
            GFX.Gui["assistmode"].DrawJustified(new Vector2(960f, (float)(540.0 + (64.0 * (double)Ease.CubeOut(fade)))), new Vector2(0.5f, 1f), iconColor * fade);
        }

        private Color SelectionColor(bool selected)
        {
            return selected ? (Settings.Instance.DisableFlashes || Scene.BetweenInterval(0.1f) ? TextMenu.HighlightColorA : TextMenu.HighlightColorB) * fade : Color.White * fade;
        }
