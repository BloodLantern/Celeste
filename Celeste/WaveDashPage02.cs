...
namespace Celeste
{
    ...
    {
        private readonly List<WaveDashPage02.TitleText> title = new();
        ...
        public override IEnumerator Routine()
        {
            ...
            Vector2 pos = new(128f, 128f);
            for (int i = 0; i < text.Length; ++i)
            {
                WaveDashPage02.TitleText item = new(pos, text[i]);
            ...
            yield return waveDashPage02.PressButton();
            _ = Audio.Play("event:/new_content/game/10_farewell/ppt_impossible");
        ...
        {
            foreach (WaveDashPage02.TitleText titleText in title)
            {
                titleText.Render();
            }
            ...
            Matrix transformMatrix = Matrix.CreateRotationZ((float)(((double)Ease.CubeIn(1f - impossibleEase) * 8.0) - 0.5)) * Matrix.CreateTranslation(Width - 500, Height - 600, 0.0f);
        ...
        private class TitleText
        {
            ...
            public void Render()
            {
                ...
                ActiveFont.DrawOutline(Text, Position + new Vector2(Width / 2f, (float)((double)ActiveFont.LineHeight * 0.5 * 1.5)), new Vector2(0.5f, 0.5f), Vector2.One * (float)(1.0 + (1.0 - (double)Ease.CubeOut(ease))) * 1.5f, Color.White, 2f, Color.Black);
            }
        }
    }
}
