...
namespace Celeste
{
    public class GlassBlockBg : Entity
    {
        ...
        private readonly GlassBlockBg.Star[] stars = new GlassBlockBg.Star[100];
        private readonly GlassBlockBg.Ray[] rays = new GlassBlockBg.Ray[50];
        ...
        public GlassBlockBg()
        {
            ...
            Add(new DisplacementRenderHook(new Action(OnDisplacementRender)));
            Depth = -9990;
            ...
            for (int index = 0; index < stars.Length; ++index)
            {
                ...
                stars[index].Texture = Calc.Random.Choose<MTexture>(atlasSubtextures);
                stars[index].Color = Calc.Random.Choose<Color>(GlassBlockBg.starColors);
            ...
            for (int index = 0; index < rays.Length; ++index)
            {
                ...
                rays[index].Length = Calc.Random.Choose<int>(48, 96, 128);
        ...
        private void BeforeRender()
        {
            ...
            Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D)starsTarget);
            ...
            for (int index = 0; index < stars.Length; ++index)
            {
                ...
                {
                    X = Mod(stars[index].Position.X - (camera.X * (1f - scroll.X)), num1),
                    Y = Mod(stars[index].Position.Y - (camera.Y * (1f - scroll.Y)), num2)
                };
            ...
            for (int index = 0; index < rays.Length; ++index)
            {
                Vector2 position = new()
                {
                    X = Mod(rays[index].Position.X - (camera.X * 0.9f), num1),
                    Y = Mod(rays[index].Position.Y - (camera.Y * 0.9f), num2)
                };
            ...
            Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D)beamsTarget);
            Engine.Graphics.GraphicsDevice.Clear(Color.Transparent);
            GFX.DrawVertices<VertexPositionColor>(Matrix.Identity, verts, vertex);
        }
        ...
        {
            Vector2 vector2_1 = new(-rayNormal.Y, rayNormal.X);
        ...
        public override void Render()
        {
            ...
            if (starsTarget != null && !starsTarget.IsDisposed)
            {
                foreach (Entity entity in entities)
                {
                    Rectangle rectangle = new((int)((double)entity.X - position.X), (int)((double)entity.Y - position.Y), (int)entity.Width, (int)entity.Height);
                    Draw.SpriteBatch.Draw((RenderTarget2D)starsTarget, entity.Position, new Rectangle?(rectangle), Color.White);
                }
            }
            ...
            {
                Rectangle rectangle = new((int)((double)entity.X - position.X), (int)((double)entity.Y - position.Y), (int)entity.Width, (int)entity.Height);
                Draw.SpriteBatch.Draw((RenderTarget2D)beamsTarget, entity.Position, new Rectangle?(rectangle), Color.White);
            }
        }
