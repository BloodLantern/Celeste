...
namespace Celeste
{
    public class Flagline : Component
    {
        ...
        private readonly Flagline.Cloth[] clothes;
            ...
            : base(true, true)
        {
            ...
            clothes = new Flagline.Cloth[10];
            for (int index = 0; index < clothes.Length; ++index)
            {
                clothes[index] = new Flagline.Cloth()
                {
        ...
        public override void Render()
        {
            ...
            float num2 = num1 / 8f;
            SimpleCurve simpleCurve1 = new(begin, end, ((end + begin) / 2f) + (Vector2.UnitY * (num2 + (float)(Math.Sin(waveTimer) * (double)num2 * 0.30000001192092896))));
            ...
            {
                Flagline.Cloth clothe = clothes[num3 % clothes.Length];
                percent += (flag ? clothe.Length : clothe.Step) / num1;
                ...
                if ((double)percent < 1.0 & flag)
                {
                    ...
                    SimpleCurve simpleCurve2 = new(vector2_1, point1, ((vector2_1 + point1) / 2f) + new Vector2(0.0f, num4 + (float)(Math.Sin((waveTimer * 2.0) + (double)percent) * (double)num4 * 0.40000000596046448)));
                    Vector2 vector2_2 = vector2_1;
                    for (float num5 = 1f; (double)num5 <= clothe.Length; ++num5)
                    {
