...
namespace Celeste
{
    ...
    public class FinalBossBeam : Entity
    {
        ...
        public FinalBossBeam Init(FinalBoss boss, Player target)
        {
            ...
            Vector2 to = Calc.ClosestPointOnLine(boss.BeamOrigin, boss.BeamOrigin + Calc.AngleToVector(angle, 2000f), target.Center) + ((target.Center - boss.BeamOrigin).Perpendicular().SafeNormalize(100f) * num);
        ...
        public override void Update()
        {
            ...
            if (chargeTimer > 0.0)
            {
                ...
                {
                    angle = Calc.Angle(boss.BeamOrigin, Calc.Approach(Calc.ClosestPointOnLine(boss.BeamOrigin, boss.BeamOrigin + Calc.AngleToVector(angle, 2000f), player.Center), player.Center, 200f * Engine.DeltaTime));
                }
        ...
        private void DissipateParticles()
        {
            ...
            for (int index1 = 0; index1 < 200; index1 += 12)
            {
                ...
                {
                    level.ParticlesFG.Emit(FinalBossBeam.P_Dissipate, vector2_2 + (vector2_1 * index1) + (vector * 2f * index2) + Calc.Random.Range(min, max), direction1);
                    level.ParticlesFG.Emit(FinalBossBeam.P_Dissipate, vector2_2 + (vector2_1 * index1) - (vector * 2f * index2) + Calc.Random.Range(min, max), direction2);
        ...
        private void PlayerCollideCheck()
        {
            ...
            Player player = (Scene.CollideFirst<Player>(from + vector2, to + vector2) ?? Scene.CollideFirst<Player>(from - vector2, to - vector2)) ?? Scene.CollideFirst<Player>(from, to);
            _ = (player?.Die((player.Center - boss.BeamOrigin).SafeNormalize()));
        }
        ...
        public override void Render()
        {
            ...
            int v = 0;
            Quad(ref v, beamOrigin, -vector2_2 + (vector2_3 * 2f), vector2_2 + (vector2_3 * 2f), vector2_2 + vector2_3, -vector2_2 + vector2_3, color, color);
            Quad(ref v, beamOrigin, -vector2_2 + vector2_3, vector2_2 + vector2_3, vector2_2, -vector2_2, color, transparent);
            Quad(ref v, beamOrigin, -vector2_2, vector2_2, vector2_2 - vector2_3, -vector2_2 - vector2_3, transparent, color);
            Quad(ref v, beamOrigin, -vector2_2 - vector2_3, vector2_2 - vector2_3, vector2_2 - (vector2_3 * 2f), -vector2_2 - (vector2_3 * 2f), color, color);
            GFX.DrawVertices<VertexPositionColor>((Scene as Level).Camera.Matrix, fade, fade.Length);
