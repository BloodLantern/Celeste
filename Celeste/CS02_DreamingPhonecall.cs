...
namespace Celeste
{
    public class CS02_DreamingPhonecall : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            yield return 0.3f;
            _ = dreamingPhonecall.ringtone.Play("event:/game/02_old_site/sequence_phone_ring_loop");
            ...
            _ = Audio.Play("event:/game/02_old_site/sequence_phone_pickup", dreamingPhonecall.player.Position);
            yield return dreamingPhonecall.payphone.Sprite.PlayRoutine("pickUp");
            yield return 1f;
            if (level.Session.Area.Mode == AreaMode.Normal)
            {
                _ = Audio.SetMusic("event:/music/lvl2/phone_loop");
            }

            dreamingPhonecall.payphone.Sprite.Play("talkPhone");
            yield return Textbox.Say("CH2_DREAM_PHONECALL", new Func<IEnumerator>(dreamingPhonecall.ShowShadowMadeline));
            if (dreamingPhonecall.evil != null)
            {
                if (level.Session.Area.Mode == AreaMode.Normal)
                {
                    _ = Audio.SetMusic("event:/music/lvl2/phone_end");
                }
            ...
            level.Shake(0.2f);
            VertexLight light = new(new Vector2(16f, -28f), Color.White, 0.0f, 32, 48);
            ...
            _ = Audio.Play("event:/game/02_old_site/sequence_phone_transform", dreamingPhonecall.payphone.Position);
            ...
            yield return 1.2f;
            level.EndCutscene();
            FadeWipe fadeWipe = new(level, false, () => EndCutscene(level));
        }
