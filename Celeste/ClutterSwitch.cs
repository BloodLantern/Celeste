...
namespace Celeste
{
    public class ClutterSwitch : Solid
    {
        ...
        private IEnumerator AbsorbRoutine(Player player)
        {
            ...
            List<MTexture> images = GFX.Game.GetAtlasSubtextures("objects/resortclutter/" + clutterSwitch.color + "_");
            for (int i = 0; i < 25; ++i)
            {
                for (int index = 0; index < 5; ++index)
                    effect.FlyClutter(target + Calc.AngleToVector(Calc.Random.NextFloat(6.28318548f), 320f), Calc.Random.Choose(images), false, 0.0f);
                level.Shake();
