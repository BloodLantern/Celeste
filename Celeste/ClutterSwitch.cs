...
namespace Celeste
{
    ...
    {
        public const float LightingAlphaAdd = 05f;
            ...
            : base(position, 32f, 16f, true)
        {
            ...
            sprite.Play("idle");
            Add(icon = new Image(GFX.Game["objects/resortclutter/icon_" + color.ToString()]));
            _ = icon.CenterOrigin();
            icon.Position = new Vector2(16f, 8f);
            Add(vertexLight = new VertexLight(new Vector2(CenterX - X, -1f), Color.Aqua, 1f, UnpressedLightRadius, UnpressedLightRadius * 2));
        }
        ...
        public override void Update()
        {
            ...
            if (HasPlayerOnTop())
            {
                ...
                MoveTowardsY(atY + (pressed ? PressedSpriteAdd : PressedSpriteAdd * 2), speedY * Engine.DeltaTime);
                targetXScale = 1.2f;
                if (!playerWasOnTop)
        ...
        private DashCollisionResults OnDashed(Player player, Vector2 direction)
        {
            if (!pressed && direction == Vector2.UnitY)
            {
                ...
                vertexLight.EndRadius = BrightLightRadius * 2;
                scene.DirectionalShake(Vector2.UnitY, 0.6f);
                scene.Particles.Emit(P_Pressed, 20, TopCenter - (Vector2.UnitY * 10f), new Vector2(16f, 8f));
                BePressed();
                sprite.Scale.X = 1.5f;
        ...
        private IEnumerator AbsorbRoutine(Player player)
        {
            ...
            _ = Alarm.Set(clutterSwitch, 3f, () =>
            {
                ...
                tween2.OnUpdate = t =>
                {
                    ...
                    vertexLight.StartRadius = (float) Math.Round(MathHelper.Lerp(start2, 24f, t.Eased));
                    vertexLight.EndRadius = (float) Math.Round(MathHelper.Lerp(end, 48f, t.Eased));
                };
                Add(tween2);
            });
            ...
            for (int i = 0; i < 25; ++i)
            {
                for (int index = 0; index < 5; ++index)
                    effect.FlyClutter(target + Calc.AngleToVector(Calc.Random.NextFloat((float) Math.PI * 2), 320f), Calc.Random.Choose(images), false, 0f);
