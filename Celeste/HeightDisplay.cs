...
namespace Celeste
{
    public class HeightDisplay : Entity
    {
        ...
        private IEnumerator Routine()
        {
            ...
            while ((heightDisplay.ease += Engine.DeltaTime / 0.15f) < 1.0)
                yield return null;
            ...
            while ((heightDisplay.pulse -= Engine.DeltaTime * 4f) > 0.0)
                yield return null;
            heightDisplay.pulse = 0.0f;
            yield return 1f;
            while ((heightDisplay.ease -= Engine.DeltaTime / 0.15f) > 0.0)
                yield return null;
        ...
        private IEnumerator CameraUp()
        {
            ...
            {
                level.Camera.Y = level.Bounds.Bottom - 180 + (float) (64.0 * (1.0 - Ease.CubeOut(p)));
                yield return null;
            }
        }
