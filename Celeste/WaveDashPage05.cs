...
namespace Celeste
{
    ...
    {
        private readonly List<WaveDashPage05.Display> displays = new();

        public WaveDashPage05()
        {
            Transition = WaveDashPage.Transitions.Spiral;
        ...
        public override void Added(WaveDashPresentation presentation)
        {
            ...
            displays.Add(new WaveDashPage05.Display(new Vector2(Width * 0.28f, Height - 600), Dialog.Get("WAVEDASH_PAGE5_INFO1"), "too_close", new Vector2(-50f, 20f)));
            displays.Add(new WaveDashPage05.Display(new Vector2(Width * 0.72f, Height - 600), Dialog.Get("WAVEDASH_PAGE5_INFO2"), "too_far", new Vector2(-50f, -35f)));
        }
        ...
        {
            foreach (WaveDashPage05.Display display in displays)
            {
                display.Update();
            }
        }
        ...
        private class Display
        {
            ...
            {
                Position = position;
                Info = FancyText.Parse(text, 896, 8, defaultColor: new Color?(Color.Black * 0.6f));
                Tutorial = new WaveDashPlaybackTutorial(tutorial, tutorialOffset, new Vector2(1f, 1f), new Vector2(1f, 1f))
                {
                    OnRender = () => Draw.Line(-64f, 20f, 64f, 20f, Color.Black)
                };
