...
namespace Celeste
{
    public class TempleMirrorPortal : Entity
    {
        ...
        private readonly TempleMirrorPortal.Debris[] debris = new TempleMirrorPortal.Debris[50];
        private Color debrisColorFrom = Calc.HexToColor("f442d4");
        private Color debrisColorTo = Calc.HexToColor("000000");
        private readonly MTexture debrisTexture = GFX.Game["particles/blob"];
        ...
        {
            base.Added(scene);
            scene.Add(curtain = new TempleMirrorPortal.Curtain(Position));
            scene.Add(new TempleMirrorPortal.Bg(Position));
        ...
        private IEnumerator OnSwitchRoutine(int side)
        {
            ...
            {
                yield return 0.1f;
                _ = Audio.Play("event:/game/05_mirror_temple/mainmirror_reveal", templeMirrorPortal.Position);
        ...
        private IEnumerator ActivateRoutine()
        {
            ...
            templeMirrorPortal.Add(new BeforeRenderHook(new Action(templeMirrorPortal.BeforeRender)));
            templeMirrorPortal.Add(new DisplacementRenderHook(new Action(templeMirrorPortal.RenderDisplacement)));
            while (true)
            {
                ...
                if ((double)debrisStart < templeMirrorPortal.debris.Length)
                {
        ...
        public override void Render()
        {
            ...
            {
                TempleMirrorPortal.Debris debri = debris[index];
                if (debri.Enabled)
                {
        ...
        {
            private readonly MirrorSurface surface;
        ...
        private class Curtain : Solid
        {
            ...
            public override void Update()
            {
                ...
                {
                    _ = player1.MoveV(Top - player1.Bottom);
                    _ = player1.MoveH(1f);
                }
                else
                {
                    ...
                    _ = player2.MoveV(Top - player2.Bottom);
                    _ = player2.MoveH(-1f);
                }
            }
