...
namespace Celeste
{
    ...
    public class Actor : Entity
    {
                ...
                for (int index2 = 0; index2 <= wiggleY; ++index2)
                {
                    if (index1 != 0 || index2 != 0)
                    {
                        for (int index3 = 1; index3 >= -1; index3 -= 2)
                        {
                            ...
                            {
                                Vector2 vector2 = new Vector2(index1 * index3, index2 * index4);
                ...
                for (int index6 = 0; index6 <= wiggleY; ++index6)
                {
                    if (index5 != 0 || index6 != 0)
                    {
                        for (int index7 = 1; index7 >= -1; index7 -= 2)
                        {
                            ...
                            {
                                Vector2 vector2 = new Vector2(index5 * index7, index6 * index8);
        ...
        public virtual bool IsRiding(JumpThru jumpThru) => !IgnoreJumpThrus && CollideCheckOutside(jumpThru, Position + Vector2.UnitY);

        public virtual bool IsRiding(Solid solid) => CollideCheck(solid, Position + Vector2.UnitY);
        ...
        {
            Vector2 nextPosition = Position + Vector2.UnitX * moveH;
        ...
        public void MoveTowardsX(float targetX, float maxAmount, Collision onCollide = null) => MoveToX(Calc.Approach(ExactPosition.X, targetX, maxAmount), onCollide);

        public void MoveTowardsY(float targetY, float maxAmount, Collision onCollide = null) => MoveToY(Calc.Approach(ExactPosition.Y, targetY, maxAmount), onCollide);

        public void MoveToX(float toX, Collision onCollide = null) => MoveH(toX - ExactPosition.X, onCollide);

        public void MoveToY(float toY, Collision onCollide = null) => MoveV(toY - ExactPosition.Y, onCollide);
