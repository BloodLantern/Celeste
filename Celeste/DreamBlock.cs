...
namespace Celeste
{
    ...
    public class DreamBlock : Solid
    {
        ...
        public override void Render()
        {
            ...
            Draw.Rect(shake + new Vector2((float) (X + (double) Width - 2.0), Y), 2f, 2f, playerHasDreamDash ? DreamBlock.activeLineColor : DreamBlock.disabledLineColor);
            Draw.Rect(shake + new Vector2(X, (float) (Y + (double) Height - 2.0)), 2f, 2f, playerHasDreamDash ? DreamBlock.activeLineColor : DreamBlock.disabledLineColor);
            Draw.Rect(shake + new Vector2((float) (X + (double) Width - 2.0), (float) (Y + (double) Height - 2.0)), 2f, 2f, playerHasDreamDash ? DreamBlock.activeLineColor : DreamBlock.disabledLineColor);
        }
            ...
            for (p = 1f; p > 0.0; p -= Engine.DeltaTime * 0.5f)
            {
                ...
                if (level.OnInterval(0.1f))
                {
                    for (int index = 0; index < (double) dreamBlock.Width; index += 4)
                        level.ParticlesFG.Emit(Strawberry.P_WingsBurst, new Vector2(dreamBlock.X + index, (float) (dreamBlock.Y + dreamBlock.Height * (double) dreamBlock.whiteHeight + 1.0)));
                }
                if (level.OnInterval(0.1f))
                    level.Shake();
