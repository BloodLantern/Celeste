...
namespace Celeste
{
    public class CS07_Credits : CutsceneEntity
    {
        ...
        public string Event;
        private readonly MTexture gradient = GFX.Gui["creditsgradient"].GetSubtexture(0, 1, 1920, 1);
        ...
        private readonly bool wasDashAssistOn;
        ...
        {
            _ = Audio.BusMuted("bus:/gameplay_sfx", new bool?(true));
            gotoEpilogue = level.Session.OldStats.Modes[0].Completed;
            gotoEpilogue = true;
            Add(new Coroutine(Routine()));
            Add(new PostUpdateHook(new Action(PostUpdate)));
        }
        ...
        private IEnumerator Routine()
        {
            ...
            cs07Credits1.Level.Background.Backdrops.Add(cs07Credits1.fillbg = new CS07_Credits.Fill());
            ...
            yield return 1.75f;
            yield return cs07Credits1.FadeTo(1f);
            _ = cs07Credits1.Level.Foreground.Backdrops.Remove(petals);
            petals = null;
            yield return 1f;
            ...
            foreach (CreditsTrigger entity in cs07Credits1.Scene.Tracker.GetEntities<CreditsTrigger>())
            {
                if (entity.Event == "BadelineOffset")
                {
                    ...
                    {
                        OverrideHairColor = new Color?(BadelineOldsite.HairColor)
                    };
                    yield return null;
            ...
            NPC oshiro = new(vector2 + new Vector2(0.0f, 4f));
            ...
            Coroutine oshiroRoutine = new(oshiro.MoveTo(oshiroTarget));
            ...
            yield return 5f;
            BirdNPC bird = new(oshiro.Position + new Vector2(280f, -160f), BirdNPC.Modes.None)
            {
                Depth = 10010
            };
            ...
            oshiro.Sprite.OnFrameChange = anim =>
            {
                ...
                Entity entity = new(oshiro.Position)
                {
                    Depth = oshiro.Depth + 1
                };
                cs07Credits.Scene.Add(entity);
                entity.Add(new Monocle.Image(GFX.Game["characters/oshiro/broom"])
                {
            ...
            while ((double)cs07Credits1.player.X > cs07Credits1.Level.Bounds.X + 160)
            {
                yield return null;
            }
            ...
            cs07Credits1.badeline.Sprite.Scale.X = -1f;
            yield return 0.5f;
            cs07Credits1.Add(new Coroutine(cs07Credits1.badeline.FloatTo(cs07Credits1.badeline.Position + new Vector2(16f, -12f), new int?(-1), false)));
            yield return 0.5f;
            cs07Credits1.player.Facing = Facings.Right;
            yield return 1.5f;
            ...
            if (!cs07Credits1.gotoEpilogue)
            {
                cs07Credits1.snow = new HiresSnow
                {
                    ...
                    AttachAlphaTo = new FadeWipe(Level, false, delegate ()
                    {
            ...
            {
                // ISSUE: reference to a compiler-generated method
                FadeWipe fadeWipe = new(Level, false, delegate ()
                {
                    base.EndCutscene(Level, true);
                });
            }
        ...
        {
            CS07_Credits cs07Credits = this;
            while ((double)cs07Credits.player.X > cs07Credits.Level.Bounds.X + 160)
            {
        ...
        private IEnumerator NextLevel(string name)
        {
            ...
            cs07Credits.Level.OnEndOfFrame += () =>
            {
                ...
                Vector2 from = new((float)left, (float)top);
        ...
        {
            while ((double)(fade = Calc.Approach(fade, value, Engine.DeltaTime * 0.5f)) != (double)value)
            {
                yield return null;
            }
        ...
        {
            CS07_Credits cs07Credits = this;
            List<Entity> dusty = new();
            float timer = 0.0f;
            Vector2 offset = oshiro.Position + new Vector2(220f, -24f);
            Vector2 start = offset;
            for (int index = 0; index < 3; ++index)
            {
                Entity entity = new(offset + new Vector2(index * 24, 0.0f))
                {
                    Depth = -50
                };
                entity.Add(new DustGraphic(true, autoExpandDust: true));
                Monocle.Image image = new(GFX.Game["decals/3-resort/brokenbox_" + ((char)(97 + index)).ToString()]);
            ...
            while (true)
            {
                for (int index = 0; index < dusty.Count; ++index)
                {
                    ...
                    entity.Y = offset.Y + ((float)Math.Sin(((double)timer * 4.0) + (index * 0.800000011920929)) * 4f);
                }
        ...
        private IEnumerator EventWaitJumpDash()
        {
            ...
            PlayerJump(-1);
            yield return 0.2f;
            player.OverrideDashDirection = new Vector2?(new Vector2(-1f, -1f));
        ...
        private IEnumerator EventWaitJumpDoubleDash()
        {
            ...
            cs07Credits.player.Facing = Facings.Right;
            yield return 1f;
            _ = cs07Credits.Level.Displacement.AddBurst(cs07Credits.player.Position, 0.4f, 8f, 32f, 0.5f);
        ...
        public override void Update()
        {
            ...
            if (player != null && player.Scene != null)
            {
                ...
                if (badeline != null && badelineAutoFloat)
                {
                    ...
                    badeline.Position = position + ((vector2 - position) * (1f - (float)Math.Pow(0.0099999997764825821, (double)Engine.DeltaTime)));
                    badeline.Sprite.Scale.X = -1f;
                }
                if (badeline != null && badelineAutoWalk)
                {
                    _ = player.GetChasePosition(Scene.TimeActive, (float)(0.34999999403953552 + (Math.Sin(walkOffset) * 0.10000000149011612)), out Player.ChaserState chaseState);
                ...
                if ((double)Math.Abs(player.Speed.X) > 90.0)
                {
        ...
        public void PostUpdate()
        {
            ...
            if (!player.OnGround())
            {
                cameraTarget.Y = (float)((((double)Level.Camera.Y * 2.0) + cameraTarget.Y) / 3.0);
            }

            Level.Camera.Position = position + ((cameraTarget - position) * (1f - (float)Math.Pow(0.0099999997764825821, (double)Engine.DeltaTime)));
        ...
        public override void OnEnd(Level level)
        {
            ...
            _ = Audio.BusMuted("bus:/gameplay_sfx", new bool?(false));
