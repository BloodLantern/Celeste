...
namespace Celeste
{
    public class Gondola : Solid
    {
        ...
        public Sprite Lever;
        private readonly Monocle.Image top;
            ...
            : base(data.Position + offset, 64f, 8f, true)
        {
            ...
            front.Y = -52f;
            Add(top = new Monocle.Image(GFX.Game["objects/gondola/top"]));
        ...
        public override void Added(Scene scene)
        {
            ...
            back.Depth = 9000;
            backImg = new Monocle.Image(GFX.Game["objects/gondola/back"]);
            backImg.Origin = new Vector2(backImg.Width / 2f, 12f);
            backImg.Y = -52f;
            back.Add(backImg);
            scene.Add(LeftCliffside = new Entity(Position + new Vector2(-124f, 0.0f)));
            Monocle.Image image1 = new(GFX.Game["objects/gondola/cliffsideLeft"]);
            _ = image1.JustifyOrigin(0.0f, 1f);
            LeftCliffside.Add(image1);
            LeftCliffside.Depth = 8998;
            scene.Add(RightCliffside = new Entity(Destination + new Vector2(144f, -104f)));
            Monocle.Image image2 = new(GFX.Game["objects/gondola/cliffsideRight"]);
            _ = image2.JustifyOrigin(0.0f, 0.5f);
            image2.Scale.X = -1f;
            ...
            if (!inCliffside)
            {
                ...
                Lever.Visible = false;
                UpdatePositions();
                JumpThru jumpThru = new(Position + new Vector2((float)(-(double)Width / 2.0), -36f), (int)Width, true)
                {
                    SurfaceSoundIndex = 28
                };
                Scene.Add(jumpThru);
        ...
        public override void Update()
        {
            if (inCliffside)
            {
                ...
                if ((double)Math.Abs(Rotation) < 0.5)
                {
                    num *= 0.5f;
                }

                if ((double)Math.Abs(Rotation) < 0.25)
                {
                    num *= 0.5f;
                }
                ...
                if ((double)Math.Abs(Rotation) < 0.019999999552965164 && (double)Math.Abs(RotationSpeed) < 0.20000000298023224)
                {
