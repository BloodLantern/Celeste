...
namespace Celeste
{
    public class Godrays : Backdrop
    {
        ...
        private readonly VertexPositionColor[] vertices = new VertexPositionColor[36];
        private int vertexCount;
        private Color rayColor = Calc.HexToColor("f52b63") * 0.5f;
        private readonly Godrays.Ray[] rays = new Godrays.Ray[6];
        private float fade;
        ...
        public override void Update(Scene scene)
        {
            ...
            for (int index1 = 0; index1 < rays.Length; ++index1)
            {
                ...
                Color color = rayColor * Ease.CubeInOut(Calc.Clamp((float)(((double)percent < 0.5 ? (double)percent : 1.0 - (double)percent) * 2.0), 0.0f, 1f)) * fade;
                if (entity != null)
                {
