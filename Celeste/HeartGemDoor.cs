...
namespace Celeste
{
    public class HeartGemDoor : Entity
    {
            ...
            : base(data.Position + offset)
        {
            ...
            Size = data.Width;
            openDistance = 32f;
            Vector2? nullable = data.FirstNodeNullable(offset);
        ...
        private IEnumerator Routine()
        {
            ...
            if (heartGemDoor.startHidden)
            {
                ...
                while (entity1 == null || Math.Abs(entity1.X - heartGemDoor.Center.X) >= 100.0);
                ...
                for (p = 0.0f; p < 1.0; p += Engine.DeltaTime * 1.2f)
                {
                    ...
                    if (dashBlock != null)
                    {
                        ...
                        if (entity2 != null && Math.Abs(entity2.X - heartGemDoor.Center.X) < 40.0)
            ...
            while (!heartGemDoor.Opened && heartGemDoor.Counter < (double) heartGemDoor.Requires)
            {
                Player entity = heartGemDoor.Scene.Tracker.GetEntity<Player>();
                if (entity != null && Math.Abs(entity.X - heartGemDoor.Center.X) < 80.0 && entity.X < (double) heartGemDoor.X)
                {
                    if (heartGemDoor.Counter == 0.0 && heartGemDoor.HeartGems > 0)
                    ...
                    heartGemDoor.Counter = Calc.Approach(heartGemDoor.Counter, target, (float) (Engine.DeltaTime * (double) heartGemDoor.Requires * 0.800000011920929));
                    int counter2 = (int) heartGemDoor.Counter;
                    if (counter1 != counter2)
                    {
                        yield return 0.1f;
                        if (heartGemDoor.Counter < (double) target)
                            Audio.Play("event:/game/09_core/frontdoor_heartfill", heartGemDoor.Position);
                    }
                }
                else
                    heartGemDoor.Counter = Calc.Approach(heartGemDoor.Counter, 0.0f, (float) (Engine.DeltaTime * (double) heartGemDoor.Requires * 4.0));
                yield return null;
            }
            yield return 0.5f;
            ...
            for (p = 0.0f; p < 1.0; p += Engine.DeltaTime)
            {
                ...
                if (p >= 0.40000000596046448 && level.OnInterval(0.1f))
                {
                    for (int index = 4; index < heartGemDoor.Size; index += 4)
                    {
                        level.ParticlesBG.Emit(HeartGemDoor.P_Shimmer, 1, new Vector2((float) (heartGemDoor.TopSolid.Left + (double) index + 1.0), heartGemDoor.TopSolid.Bottom - 2f), new Vector2(2f, 2f), -1.57079637f);
                        level.ParticlesBG.Emit(HeartGemDoor.P_Shimmer, 1, new Vector2((float) (heartGemDoor.BotSolid.Left + (double) index + 1.0), heartGemDoor.BotSolid.Top + 2f), new Vector2(2f, 2f), 1.57079637f);
                    }
                }
                yield return null;
            }
        ...
        public void RenderBloom()
        {
            if (Opened || !Visible)
                return;
            DrawBloom(new Rectangle((int) TopSolid.X, (int) TopSolid.Y, Size, (int) (TopSolid.Height + (double) BotSolid.Height)));
        }
        ...
        private void DrawMist(Rectangle bounds, Vector2 mist)
        {
            ...
            for (int index1 = 0; index1 < bounds.Width; index1 += num1)
            {
                ...
                {
                    mtexture.GetSubtexture((int) Mod(mist.X, num1), (int) Mod(mist.Y, num2), Math.Min(num1, bounds.Width - index1), Math.Min(num2, bounds.Height - index2), temp);
        ...
        private void DrawEdges(Rectangle bounds, Color color)
        {
            ...
            if (height > 0)
            {
                mtexture1.GetSubtexture(0, 8 - height, 7, height, temp);
            ...
            {
                mtexture1.GetSubtexture(0, 0, 8, Math.Min(8, bounds.Height - index), temp);
        ...
        public override void Render()
        {
            ...
            {
                Rectangle bounds = new Rectangle((int) TopSolid.X, (int) TopSolid.Y, Size, (int) (TopSolid.Height + (double) BotSolid.Height));
                DrawInterior(bounds);
            ...
            else
            {
                if (TopSolid.Visible)
                {
                    Rectangle bounds = new Rectangle((int) TopSolid.X, (int) TopSolid.Y, Size, (int) TopSolid.Height);
                    DrawInterior(bounds);
                    DrawEdges(bounds, color1);
                }
                if (BotSolid.Visible)
                {
                    Rectangle bounds = new Rectangle((int) BotSolid.X, (int) BotSolid.Y, Size, (int) BotSolid.Height);
                    DrawInterior(bounds);
                    DrawEdges(bounds, color1);
                }
            }
            ...
            for (int index1 = 0; index1 < num3; ++index1)
            {
                ...
                for (int index2 = 0; index2 < num4; ++index2)
                {
                    ...
                    icon[(int) (Ease.CubeIn(Calc.ClampedMap(Counter, min, min + 1f)) * (double) (icon.Count - 1))].DrawCentered(vector2 + new Vector2(index2 * num1, 0.0f), color2);
                }
            }
        }
        ...
        private class WhiteLine : Entity
        {
            ...
            public override void Update()
            {
                ...
                for (float left = (int) level.Camera.Left; left < (double) level.Camera.Right; ++left)
                {
                    if (left < (double) X || left >= X + (double) blockSize)
