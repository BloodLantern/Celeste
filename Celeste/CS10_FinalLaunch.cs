...
namespace Celeste
{
    public class CS10_FinalLaunch : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene()
        {
            ...
            float p1;
            for (p1 = 0.0f; p1 < 1.0; p1 += Engine.DeltaTime / 12f)
            {
            ...
            while (cs10FinalLaunch.bird != null)
                yield return null;
            FadeWipe wipe = new FadeWipe(cs10FinalLaunch.Level, false);
            wipe.Duration = 4f;
            ScreenWipe.WipeColor = Color.White;
            ...
            int to = 180;
            for (float p2 = 0.0f; p2 < 1.0; p2 += Engine.DeltaTime / 2f)
            {
        ...
        private IEnumerator BirdRoutine(float delay)
        {
            ...
            for (t = 0.0f; t < 1.0; t += Engine.DeltaTime / 2f)
            {
            ...
            for (t = 0.0f; t < 1.0; t += Engine.DeltaTime / 4f)
            {
                if (t >= 0.34999999403953552 && !playedAnim)
                {
                    cs10FinalLaunch.bird.Sprite.Play("flyupRoll");
                    playedAnim = true;
                }
