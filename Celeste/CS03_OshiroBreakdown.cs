...
namespace Celeste
{
    public class CS03_OshiroBreakdown : CutsceneEntity
    {
        ...
        private readonly List<DustStaticSpinner> creatures = new();
        private readonly List<Vector2> creatureHomes = new();
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            yield return Textbox.Say("CH3_OSHIRO_BREAKDOWN", new Func<IEnumerator>(cs03OshiroBreakdown.WalkLeft), new Func<IEnumerator>(cs03OshiroBreakdown.WalkRight), new Func<IEnumerator>(cs03OshiroBreakdown.CreateDustA), new Func<IEnumerator>(cs03OshiroBreakdown.CreateDustB));
        ...
        private IEnumerator CreateDustA()
        {
            ...
            _ = Audio.Play("event:/char/oshiro/chat_collapse", cs03OshiroBreakdown.oshiro.Position);
        ...
        private IEnumerator CreateDustB()
        {
            ...
            yield return 1f;
            _ = Audio.Play("event:/char/oshiro/chat_get_up", cs03OshiroBreakdown.oshiro.Position);
        ...
        private IEnumerator MoveDust(DustStaticSpinner creature, Vector2 to)
        {
            ...
            SimpleCurve curve = new(begin, to, ((to + begin) / 2f) + (Vector2.UnitY * (Calc.Random.NextFloat(60f) - 30f)));
            for (float p = 0.0f; (double)p < 1.0; p += Engine.DeltaTime)
            {
                yield return null;
