...
namespace Celeste
{
    public class OuiJournal : Oui
    {
        ...
        public override IEnumerator Enter(Oui from)
        {
            ...
            double num2 = journal.Overworld.Mountain.EaseCamera(journal.Overworld.Mountain.Area, journal.cameraEnd, 2f);
            journal.Overworld.Mountain.AllowUserRotation = false;
            for (float p = 0.0f; p < 1.0; p += Engine.DeltaTime / 0.4f)
            {
                journal.rotation = -0.025f * Ease.BackOut(p);
                journal.X = (float) (1920.0 * Ease.CubeInOut(p) - 1920.0);
                journal.dotEase = p;
                yield return null;
            }
            journal.dotEase = 1f;
        ...
        public override IEnumerator Leave(Oui next)
        {
            ...
            double num = ouiJournal.Overworld.Mountain.EaseCamera(ouiJournal.Overworld.Mountain.Area, ouiJournal.cameraStart, 0.4f);
        ...
        private IEnumerator EaseOut(float duration)
        {
            ...
            for (float p = 0.0f; p < 1.0; p += Engine.DeltaTime / duration)
            {
                ...
                ouiJournal.X = (float) (0.0 + -1920.0 * Ease.CubeInOut(p));
        ...
        public override void Render()
        {
            ...
            if (turningPage)
            {
                ...
                Draw.SpriteBatch.Draw((RenderTarget2D) NextPageBuffer, position, NextPageBuffer.Bounds, Color.White, rotation, Vector2.Zero, Vector2.One, SpriteEffects.None, 0.0f);
            }
            if (turningPage && num1 > 0.0)
            ...
            if (x1 > 0.0)
            {
                ...
                Draw.SpriteBatch.Draw((RenderTarget2D) CurrentPageBuffer, position, CurrentPageBuffer.Bounds, Color.White, rotation, Vector2.Zero, new Vector2(x1, 1f), SpriteEffects.None, 0.0f);
            }
            if (Pages.Count <= 0)
                return;
            ...
            GFX.Gui["dotarrow_outline"].DrawCentered(vector2 + new Vector2(-num2 / 2 - 50, (float) (32.0 * (1.0 - Ease.CubeOut(leftArrowEase)))), Color.White * leftArrowEase, new Vector2(-1f, 1f));
            GFX.Gui["dotarrow_outline"].DrawCentered(vector2 + new Vector2(num2 / 2 + 50, (float) (32.0 * (1.0 - Ease.CubeOut(rightArrowEase)))), Color.White * rightArrowEase);
        }
    }
}
