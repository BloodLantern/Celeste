...
namespace Celeste
{
    public class RainFG : Backdrop
    {
        ...
        public override void Render(Scene scene)
        {
            ...
            for (int index = 0; index < particles.Length; ++index)
            {
                Vector2 position = new Vector2(mod((float) (particles[index].Position.X - (double) camera.X - 32.0), 384f), mod((float) (particles[index].Position.Y - (double) camera.Y - 32.0), 244f));
        ...
        private struct Particle
        {
            ...
            public void Init()
            {
                ...
                Scale = new Vector2((float) (4.0 + (Speed.Length() - 200.0) / 400.0 * 12.0), 1f);
            }
        }
    }
}
