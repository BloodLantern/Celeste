...
namespace Celeste
{
    ...
    public class Textbox : Entity
    {
        ...
        private readonly Func<IEnumerator>[] events;
        private readonly Coroutine runRoutine;
        private readonly Coroutine skipRoutine;
        ...
        private readonly Sprite portraitSprite = new(null, null);
        private bool portraitExists;
        private bool portraitIdling;
        private float portraitScale = 1.5f;
        private readonly Wiggler portraitWiggle;
        private Sprite portraitGlitchy;
        private bool isPortraitGlitchy;
        private readonly Dictionary<string, SoundSource> talkers = new();
        private SoundSource activeTalker;
        private readonly SoundSource phonestatic;
                ...
                switch (current)
                {
                    ...
                    case FancyText.Portrait _:
                        ...
                        if (textbox.portraitExists)
                        {
                            ...
                            {
                                _ = GFX.PortraitsSpriteBank.CreateOn(textbox.portraitSprite, next1.SpriteId);
                            ...
                            if (!string.IsNullOrEmpty(str))
                            {
                                if (str == "ex")
                                {
                                    _ = textbox.phonestatic.Play("event:/char/dialogue/sfx_support/phone_static_ex");
                                }
                                else if (str == "mom")
                                {
                                    _ = textbox.phonestatic.Play("event:/char/dialogue/sfx_support/phone_static_mom");
                                }
                            }
                    ...
                    case FancyText.Char _:
                        ...
                        if (last is not null and FancyText.NewPage)
                        {
                            --textbox.index;
                            yield return 0.2f;
                            ++textbox.index;
        ...
        private void StopTalker()
        {
            ...
            _ = activeTalker.Param("dialogue_portrait", 0.0f);
            _ = activeTalker.Param("dialogue_end", 1f);
        }
        ...
        private void StartTalker()
        {
            ...
            _ = activeTalker.Param("dialogue_portrait", portrait != null ? portrait.SfxExpression : 1f);
            _ = activeTalker.Param("dialogue_end", 0.0f);
        }
        ...
        private IEnumerator EaseOpen()
        {
            ...
            if (textbox1.ease < 1.0)
            {
                ...
                _ = textbox1.portrait != null && textbox1.portrait.Sprite.IndexOf("madeline", StringComparison.InvariantCultureIgnoreCase) >= 0
        ...
        {
            return autoPressContinue || ((Input.MenuConfirm.Pressed || Input.MenuCancel.Pressed) && !disableInput);
        }
        ...
        public override void Render()
        {
            ...
            if (RenderOffset == Vector2.Zero)
            {
                ...
                vector2_1.Y += (int)(136.0 * (1.0 - (double)num1));
            }
            ...
            if (portraitExists)
            {
                ...
                portraitSprite.Scale.Y = (float)(portraitScale * (((272.0 * (double)num1) - 32.0) / 240.0) * (portrait.UpsideDown ? -1.0 : 1.0));
                Sprite portraitSprite1 = portraitSprite;
                portraitSprite1.Scale *= (float)(0.89999997615814209 + ((double)portraitWiggle.Value * 0.10000000149011612));
                Sprite portraitSprite2 = portraitSprite;
                portraitSprite2.Position += new Vector2(120f, (float)(272.0 * (double)num1 * 0.5));
                portraitSprite.Color = Color.White * num1;
                if ((double)Math.Abs(portraitSprite.Scale.Y) > 0.05000000074505806)
                {
            ...
            Vector2 vector2_2 = new(innerTextPadding + (portrait == null || PortraitSide(portrait) >= 0 ? 0.0f : 256f), innerTextPadding);
        ...
        {
            Textbox textbox = new(dialog, events);
