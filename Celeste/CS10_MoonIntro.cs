    ...
    public class CS10_MoonIntro : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            if (cs10MoonIntro.bird != null)
            {
                ...
                _ = level.Session.DoNotLoad.Add(cs10MoonIntro.bird.EntityID);
            ...
            while ((double)cs10MoonIntro.player.Top > level.Bounds.Bottom)
            {
                yield return null;
            }

            yield return 0.2f;
            _ = Audio.Play("event:/new_content/char/madeline/screenentry_lowgrav", cs10MoonIntro.player.Position);
            ...
            while (!cs10MoonIntro.player.OnGround() && (double)cs10MoonIntro.player.Bottom < level.Bounds.Bottom)
            {
                _ = cs10MoonIntro.player.MoveVExact(16);
            }

            cs10MoonIntro.player.StateMachine.State = 11;
            yield return 0.5f;
            yield return cs10MoonIntro.BadelineAppears();
            yield return Textbox.Say("CH9_LANDING", new Func<IEnumerator>(cs10MoonIntro.BadelineTurns), new Func<IEnumerator>(cs10MoonIntro.BadelineVanishes));
        ...
        private IEnumerator BadelineTurns()
        {
            ...
            Wiggler wiggler = Wiggler.Create(0.5f, 3f, v => badeline.Sprite.Scale = new Vector2(target, 1f) * (float)(1.0 + (0.20000000298023224 * (double)v)), true, true);
            cs10MoonIntro.Add(wiggler);
            _ = Audio.Play(target < 0 ? "event:/char/badeline/jump_wall_left" : "event:/char/badeline/jump_wall_left", cs10MoonIntro.badeline.Position);
            yield return 0.6f;
        }
        ...
        private IEnumerator BadelineAppears()
        {
            ...
            _ = cs10MoonIntro.Level.Displacement.AddBurst(cs10MoonIntro.player.Center, 0.5f, 8f, 32f, 0.5f);
            _ = Audio.Play("event:/char/badeline/maddy_split", cs10MoonIntro.player.Position);
            cs10MoonIntro.badeline.Sprite.Scale.X = 1f;
            yield return cs10MoonIntro.badeline.FloatTo(cs10MoonIntro.player.Position + new Vector2(-16f, -16f), new int?(1), false);
        ...
        public override void OnEnd(Level level)
        {
            ...
            {
                bird.RemoveSelf();
                _ = level.Session.DoNotLoad.Add(bird.EntityID);
            }
