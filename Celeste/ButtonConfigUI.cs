...
namespace Celeste
{
    ...
    public class ButtonConfigUI : TextMenu
    {
        ...
        private void AddMap(string label, Binding binding)
        {
            ...
            Add(new Setting(txt, binding, true).Pressed(() =>
            {
                remappingText = txt;
                Remap(binding);
        ...
        public override void Update()
        {
            ...
            base.Update();
            Focused = !closing && inputDelay <= 0.0 && !waitingForController && !remapping;
            if (!closing)
            {
            ...
            {
                if (Input.ESC.Pressed || timeout <= 0.0 || !Input.GuiInputController())
                {
                    remapping = false;
                    Focused = true;
                }
        ...
        public override void Render()
        {
            ...
            if (MInput.GamePads[Input.Gamepad].Attached)
            {
                ...
                if (remappingEase > 0.0)
                {
                    ...
                    ActiveFont.Draw(Dialog.Get("BTN_CONFIG_CHANGING"), position + new Vector2(0.0f, -8f), new Vector2(0.5f, 1f), Vector2.One * 0.7f, Color.LightGray * Ease.CubeIn(remappingEase));
                    ActiveFont.Draw(remappingText, position + new Vector2(0.0f, 8f), new Vector2(0.5f, 0.0f), Vector2.One * 2f, Color.White * Ease.CubeIn(remappingEase));
                }
            }
            else
