...
namespace Celeste.Pico8
{
    public class Classic
    {
        ...
        private int level_index() => room.X % 8 + room.Y * 8;

        private bool is_title() => level_index() == 31;
        ...
        private void break_spring(spring obj) => obj.hide_in = 15;
        ...
        public void load_room(int x, int y)
        {
            ...
            for (int index1 = 0; index1 <= 15; ++index1)
            {
                for (int index2 = 0; index2 <= 15; ++index2)
                {
                    ...
                    {
                        case 11:
                            init_object(new platform(), index1 * 8, index2 * 8).dir = -1f;
                            break;
                        case 12:
                            init_object(new platform(), index1 * 8, index2 * 8).dir = 1f;
                            break;
                        default:
                            ...
                            {
                                init_object(classicObject, index1 * 8, index2 * 8, num);
                            }
                            break;
                    }
                }
            ...
            if (is_title())
                return;
            init_object(new room_title(), 0.0f, 0.0f);
        }
        ...
        public void Update()
        {
            ...
            else
            {
                ...
                while (num != -1)
                {
                    ...
                    {
                        ClassicObject classicObject = objects[index];
                        if (classicObject != null)
                        {
        ...
        public void Draw()
        {
            ...
            {
                DeadParticle deadParticle = dead_particles[index];
            ...
            if (level_index() != 30)
                return;
            ClassicObject classicObject1 = null;
            foreach (ClassicObject classicObject2 in objects)
            {
                if (classicObject2 is player)
                {
        ...
        private void draw_object(ClassicObject obj) => obj.draw();
        ...
        private float clamp(float val, float a, float b) => E.max(a, E.min(b, val));

        private float appr(float val, float target, float amount) => val <= (double)target ? E.min(val + amount, target) : E.max(val - amount, target);
        ...
        private bool maybe() => E.rnd(1f) < 0.5;

        private bool solid_at(float x, float y, float w, float h) => tile_flag_at(x, y, w, h, 0);

        private bool ice_at(float x, float y, float w, float h) => tile_flag_at(x, y, w, h, 4);

        private bool tile_flag_at(float x, float y, float w, float h, int flag)
        {
            for (int x1 = (int)E.max(0.0f, E.flr(x / 8f));
                x1 <= (double)E.min(15f, (float)((x + (double)w - 1.0) / 8.0)); ++x1)
            {
                for (int y1 = (int)E.max(0.0f, E.flr(y / 8f));
                    y1 <= (double)E.min(15f, (float)((y + (double)h - 1.0) / 8.0)); ++y1)
                {
        ...
        private int tile_at(int x, int y) => E.mget(room.X * 16 + x, room.Y * 16 + y);
        ...
        private bool spikes_at(float x, float y, int w, int h, float xspd, float yspd)
        {
                ...
                x1 <= (double)E.min(15f, (float)(((double)x + w - 1.0) / 8.0)); ++x1)
            {
                    ...
                    y1 <= (double)E.min(15f, (float)(((double)y + h - 1.0) / 8.0)); ++y1)
                {
                    ...
                    if (num == 17 && (E.mod((float)((double)y + h - 1.0), 8f) >= 6.0 || (double)y + h == y1 * 8 + 8) && yspd >= 0.0 || num == 27 && E.mod(y, 8f) <= 2.0 && yspd <= 0.0 || num == 43 && E.mod(x, 8f) <= 2.0 && xspd <= 0.0 || num == 59 && (((double)x + w - 1.0) % 8.0 >= 6.0 || (double)x + w == x1 * 8 + 8) && xspd >= 0.0)
                        return true;
                }
            }
            return false;
        ...
        public class player : ClassicObject
        {
            ...
            public override void update()
            {
                ...
                else
                {
                    ...
                    if ((double)E.abs(spd.X) > num2)
                        spd.X = G.appr(spd.X, E.sign(spd.X) * num2, amount2);
                    else
                        spd.X = G.appr(spd.X, ox * num2, amount1);
                    if (spd.X != 0.0)
                        flipX = spd.X < 0.0;
                    float target = 2f;
                    float amount3 = 0.21f;
                    if (E.abs(spd.Y) <= 0.15000000596046448)
                        amount3 *= 0.5f;
                    if (ox != 0 && is_solid(ox, 0) && !is_ice(ox, 0))
                    {
                        target = 0.4f;
                        if (E.rnd(10f) < 2.0)
                            G.init_object(new smoke(), x + ox * 6, y);
                    }
                    if (!flag1)
                    ...
                    if (jbuffer > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (num3 != 0)
                            {
                                ...
                                spd.X = -num3 * (num2 + 1);
                                if (!is_ice(num3 * 3, 0))
                                    G.init_object(new smoke(), x + num3 * 6, y);
                            }
                        }
                    }
                    int num4 = 5;
                ...
                if (!flag1)
                {
                    ...
                    spr = 7f;
                else if (spd.X == 0.0 || !E.btn(G.k_left) && !E.btn(G.k_right))
                    spr = 1f;
                else
                    spr = (float)(1.0 + spr_off % 4.0);
            ...
            public override void update()
            {
                ...
                else if (spr == 18.0)
                {
                    ...
                    if (player != null && player.spd.Y >= 0.0)
                    {
                        ...
                        delay = 10;
                        G.init_object(new smoke(), x, y);
                        fall_floor fallFloor = collide<fall_floor>(0, 1);
                        if (fallFloor != null)
        ...
        public class key : ClassicObject
        {
            ...
            {
                int num1 = E.flr(spr);
                spr = (float)(9.0 + (E.sin(G.frames / 30f) + 0.5) * 1.0);
                int num2 = E.flr(spr);
        ...
        public class platform : ClassicObject
        {
            ...
            public override void update()
            {
                ...
                if (x > 128.0)
                    x = -16f;
                if (!check<player>(0, 0))
                    collide<player>(0, -1)?.move_x((int)(x - (double)last), 1);
                last = x;
            }
        ...
        public class orb : ClassicObject
        {
            ...
            public override void draw()
            {
                ...
                for (int index = 0; index <= 7; ++index)
                    E.circfill((float)(x + 4.0 + E.cos(num + index / 8f) * 8.0), (float)(y + 4.0 + E.sin(num + index / 8f) * 8.0), 1f, 7f);
            }
        }
        ...
        public class ClassicObject
        {
            ...
            public bool is_solid(int ox, int oy) => oy > 0 && !check<platform>(ox, 0) && check<platform>(ox, oy) || G.solid_at(x + hitbox.X + ox, y + hitbox.Y + oy, hitbox.Width, hitbox.Height) || check<fall_floor>(ox, oy) || check<fake_wall>(ox, oy);

            public bool is_ice(int ox, int oy) => G.ice_at(x + hitbox.X + ox, y + hitbox.Y + oy, hitbox.Width, hitbox.Height);

            public T collide<T>(int ox, int oy) where T : ClassicObject
            {
                Type type = typeof(T);
                foreach (ClassicObject classicObject in G.objects)
                {
            ...
            public bool check<T>(int ox, int oy) where T : ClassicObject => collide<T>(ox, oy) != null;
