...
namespace Celeste.Pico8
{
    public class Classic
    {
        ...
        private void draw_time(int x, int y)
        {
            ...
            this.E.print((num2 < 10 ? (object)
                "0" : (object)
                "").ToString() + (object)num2 + ":" + (num1 < 10 ? (object)
                "0" : (object)
                "") + (object)num1 + ":" + (seconds < 10 ? (object)
                "0" : (object)
                "") + (object)seconds, (float)(x + 1), (float)(y + 1), 7f);
        }
        ...
        {
            for (int x1 = (int)this.E.max(0.0f, (float)this.E.flr(x / 8f));
                (double)x1 <= (double)this.E.min(15f, (float)(((double)x + (double)w - 1.0) / 8.0)); ++x1)
            {
                for (int y1 = (int)this.E.max(0.0f, (float)this.E.flr(y / 8f));
                    (double)y1 <= (double)this.E.min(15f, (float)(((double)y + (double)h - 1.0) / 8.0)); ++y1)
                {
        ...
        public class message : Classic.ClassicObject
        {
            ...
            public override void draw()
            {
                ...
                if (this.check<Classic.player>(4, 0))
                {
                    ...
                    for (int index = 0;
                        (double)index < (double)this.index; ++index)
                    {
                        if (str[index] != '#')
                        {
        ...
        public class ClassicObject
        {
            ...
            public void move_x(int amount, int start)
            {
                if (this.solids)
                {
                    ...
                    for (int index = start;
                        (double)index <= (double)this.E.abs((float)amount); ++index)
                    {
                        if (!this.is_solid(ox, 0))
                        {
                            this.x += (float)ox;
