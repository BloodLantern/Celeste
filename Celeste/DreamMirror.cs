...
namespace Celeste
{
    public class DreamMirror : Entity
    {
        ...
        public IEnumerator BreakRoutine(int direction)
        {
            ...
            while (Math.Abs(dreamMirror.reflection.X - dreamMirror.breakingGlass.Width / 2f) > 3.0)
            {
            ...
            for (float x = (float) (-(double) dreamMirror.breakingGlass.Width / 2.0); x < dreamMirror.breakingGlass.Width / 2.0; x += 8f)
            {
                for (float y = -dreamMirror.breakingGlass.Height; y < 0.0; y += 8f)
                {
            ...
            while (Math.Abs(dreamMirror.badeline.X - dreamMirror.X) < 60.0)
            {
            ...
            while (Math.Abs(dreamMirror.badeline.X - dreamMirror.X) < 128.0)
            {
                speed += (float) (Engine.DeltaTime * (double) -direction * 128.0);
                dreamMirror.badeline.X += speed * Engine.DeltaTime;
                dreamMirror.badeline.Y -= (float) (Math.Abs(speed) * (double) Engine.DeltaTime * 0.800000011920929);
                yield return null;
            }
