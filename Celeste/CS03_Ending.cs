...
namespace Celeste
{
    public class CS03_Ending : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            cs03Ending1.Add(new Coroutine(cs03Ending1.player.DummyRunTo((float) (cs03Ending1.roof.X + (double) cs03Ending1.roof.Width - 32.0), true)));
            yield return null;
            ...
            yield return 0.5f;
            yield return Textbox.Say("CH3_OSHIRO_CHASE_END", cs03Ending1.GhostSmash);
            ...
            while (cs03Ending1.oshiro.Y != (double) target)
            {
                cs03Ending1.oshiro.Y = Calc.Approach(cs03Ending1.oshiro.Y, target, 6f * Engine.DeltaTime);
                yield return null;
            }
            yield return 0.6f;
            yield return Textbox.Say("CH3_ENDING", cs03Ending1.OshiroTurns);
            ...
            while (cs03Ending1.oshiro.X > (double) (level.Bounds.Left - 16))
            {
                cs03Ending1.oshiro.X -= 40f * Engine.DeltaTime;
                cs03Ending1.oshiroSprite.Y = (float) Math.Sin(t += Engine.DeltaTime * 2f) * 2f;
        ...
        private IEnumerator GhostSmash(float topDelay, bool final)
        {
            ...
            if (cs03Ending.angryOshiro != null)
            {
                ...
                if (!final)
                {
                    ...
                    cs03Ending.angryOshiro.Y = (float) ((ground + (double) from) / 2.0);
                if (cs03Ending.angryOshiro != null)
                {
