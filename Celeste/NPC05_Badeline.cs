    ...
    public class NPC05_Badeline : NPC
    {
        ...
        private IEnumerator FirstScene()
        {
            ...
            while (true)
            {
                ...
                if (player == null || player.X <= npC05Badeline.X - 64.0 || player.Y <= npC05Badeline.Y - 32.0)
                    yield return null;
                else
                    break;
            }
            ...
            while (true)
            {
                do
                {
                    double x = npC05Badeline.shadow.X;
                    bounds = npC05Badeline.Level.Bounds;
                ...
                while (player.X <= npC05Badeline.shadow.X - 24.0);
...
label_12:
        ...
        private IEnumerator SecondScene(int startIndex)
        {
            ...
            while (index < npc.nodes.Length)
            {
                ...
                while (player == null || (player.Position - npc.shadow.Position).Length() > 70.0)
                    yield return null;
        ...
        public void MoveToNode(int index, bool chatMove = true)
        {
            ...
            tween.OnUpdate = t =>
            {
                ...
                if (t.Eased < 0.10000000149011612 || t.Eased > 0.89999997615814209 || !Scene.OnInterval(0.05f))
                    return;
        ...
        public void SnapToNode(int index) => shadow.Position = nodes[index];
    }
}
