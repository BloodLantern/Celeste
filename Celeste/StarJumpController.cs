...
namespace Celeste
{
    ...
    public class StarJumpController : Entity
    {
        ...
        private readonly VertexPositionColor[] vertices = new VertexPositionColor[600];
        private int vertexCount;
        private Color rayColor = Calc.HexToColor("a3ffff") * 0.25f;
        private readonly StarJumpController.Ray[] rays = new StarJumpController.Ray[100];
        ...
        public override void Added(Scene scene)
        {
            ...
            _ = level.Session.Audio.Music.Layer(1, 1f);
            _ = level.Session.Audio.Music.Layer(2, 0.0f);
            _ = level.Session.Audio.Music.Layer(3, 0.0f);
            _ = level.Session.Audio.Music.Layer(4, 0.0f);
        ...
        public override void Update()
        {
            ...
            if (entity != null)
            {
                ...
                _ = level.Session.Audio.Music.Layer(1, Calc.ClampedMap(centerY, maxY, minY, 1f, 0.0f));
                _ = level.Session.Audio.Music.Layer(2, Calc.ClampedMap(centerY, maxY, minY));
                level.Session.Audio.Apply();
        ...
        private void UpdateBlockFill()
        {
            ...
            Vector2 vector2_1 = new(-vector.Y, vector.X);
            int num1 = 0;
            for (int index1 = 0; index1 < rays.Length; ++index1)
            {
                ...
                Vector2 vector2_2 = new((int)x, (int)y);
                Color color = rayColor * Ease.CubeInOut(Calc.YoYo(percent));
                VertexPositionColor vertexPositionColor1 = new(new Vector3(vector2_2 + (vector2_1 * width) + (vector * length), 0.0f), color);
                VertexPositionColor vertexPositionColor2 = new(new Vector3(vector2_2 - (vector2_1 * width), 0.0f), color);
                VertexPositionColor vertexPositionColor3 = new(new Vector3(vector2_2 + (vector2_1 * width), 0.0f), color);
                VertexPositionColor vertexPositionColor4 = new(new Vector3(vector2_2 - (vector2_1 * width) - (vector * length), 0.0f), color);
        ...
        private void BeforeRender()
        {
            ...
            GFX.DrawVertices<VertexPositionColor>(Matrix.Identity, vertices, vertexCount);
        }
        ...
        private struct Ray
        {
            ...
            public void Reset()
            {
                ...
                Duration = (float)(4.0 + ((double)Calc.Random.NextFloat() * 8.0));
