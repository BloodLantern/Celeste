...
namespace Celeste
{
    public class CrushBlock : Solid
    {
        ...
        private readonly List<CrushBlock.MoveState> returnStack;
        ...
        private readonly List<Monocle.Image> idleImages = new();
        private readonly List<Monocle.Image> activeTopImages = new();
        private readonly List<Monocle.Image> activeRightImages = new();
        private readonly List<Monocle.Image> activeLeftImages = new();
        private readonly List<Monocle.Image> activeBottomImages = new();
            ...
            : base(position, width, height, false)
        {
            ...
            returnStack = new List<CrushBlock.MoveState>();
            this.chillOut = chillOut;
            giant = (((double)Width < 48.0 ? 0 : ((double)Height >= 48.0 ? 1 : 0)) & (chillOut ? 1 : 0)) != 0;
            canActivate = true;
            ...
            int x1 = (int)((double)Width / 8.0) - 1;
            int y1 = (int)((double)Height / 8.0) - 1;
            ...
            {
                AddImage(idle, x2, 0, Calc.Random.Choose<int>(1, 2), 0, borderY: -1);
                AddImage(idle, x2, y1, Calc.Random.Choose<int>(1, 2), 3, borderY: 1);
            }
            for (int y2 = 1; y2 < y1; ++y2)
            {
                AddImage(idle, 0, y2, 0, Calc.Random.Choose<int>(1, 2), -1);
                AddImage(idle, x1, y2, 3, Calc.Random.Choose<int>(1, 2), 1);
            }
        ...
        public CrushBlock(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Width, data.Height, data.Enum<CrushBlock.Axes>("axes"), data.Bool("chillout"))
        {
        }
        ...
        public override void Update()
        {
            ...
            _ = (currentMoveLoopSfx?.Param("submerged", Submerged ? 1f : 0.0f));
            if (returnLoopSfx == null)
            {
                return;
            }

            _ = returnLoopSfx.Param("submerged", Submerged ? 1f : 0.0f);
        }
            ...
            int borderY = 0)
        {
            ...
            Vector2 vector2 = new(x * 8, y * 8);
            if (borderX != 0)
            {
                Monocle.Image image = new(subtexture)
                {
                    Color = Color.Black,
                    Position = vector2 + new Vector2(borderX, 0.0f)
                };
                Add(image);
            }
            if (borderY != 0)
            {
                ...
                {
                    Color = Color.Black,
                    Position = vector2 + new Vector2(0.0f, borderY)
                };
                Add(image);
            }
            Monocle.Image image1 = new(subtexture)
            {
                Position = vector2
            };
            Add(image1);
            ...
            if (borderX < 0)
            {
                Monocle.Image image2 = new(GFX.Game["objects/crushblock/lit_left"].GetSubtexture(0, ty * 8, 8, 8));
            ...
            {
                Monocle.Image image3 = new(GFX.Game["objects/crushblock/lit_right"].GetSubtexture(0, ty * 8, 8, 8));
            ...
            if (borderY < 0)
            {
                Monocle.Image image4 = new(GFX.Game["objects/crushblock/lit_top"].GetSubtexture(tx * 8, 0, 8, 8));
            ...
            else
            {
                ...
                Monocle.Image image5 = new(GFX.Game["objects/crushblock/lit_bottom"].GetSubtexture(tx * 8, 0, 8, 8));
        ...
        {
            return (!giant || direction.X > 0.0) && canActivate && crushDir != direction && (direction.X == 0.0 || canMoveHorizontally) && (direction.Y == 0.0 || canMoveVertically);
        }

        private void Attack(Vector2 direction)
        {
            _ = Audio.Play("event:/game/06_reflection/crushblock_activate", Center);
            if (currentMoveLoopSfx != null)
            {
                _ = currentMoveLoopSfx.Param("end", 1f);
                SoundSource sfx = currentMoveLoopSfx;
                _ = Alarm.Set(this, 0.5f, sfx.RemoveSelf);
            }
            Add(currentMoveLoopSfx = new SoundSource());
            currentMoveLoopSfx.Position = new Vector2(Width, Height) / 2f;
            _ = SaveData.Instance != null && SaveData.Instance.Name != null && SaveData.Instance.Name.StartsWith("FWAHAHA", StringComparison.InvariantCultureIgnoreCase)
            ...
            {
                CrushBlock.MoveState moveState = returnStack[returnStack.Count - 1];
            ...
            returnStack.Add(new CrushBlock.MoveState(Position, crushDir));
        }
        ...
        private IEnumerator AttackSequence()
        {
            ...
            while (true)
            {
                ...
                else if (slowing || crushBlock.CollideCheck<SolidTiles>(crushBlock.Position + (crushBlock.crushDir * 256f)))
                {
                    ...
                    {
                        slowing = true;
                        _ = Alarm.Set(crushBlock, 0.5f, () =>
                        {
                            face.Play("hurt");
                            _ = currentMoveLoopSfx.Stop();
                            TurnOffImages();
                        });
                    }
                }
                ...
                if ((double)crushBlock.Top < crushBlock.level.Bounds.Bottom + 32)
                {
                    if (!flag)
                    {
        ...
        label_13:
            ...
            if (crushBlock.crushDir == -Vector2.UnitX)
            {
                ...
                for (int index = 0; index < (double)crushBlock.Height / 8.0; ++index)
                {
                    Vector2 point = new(crushBlock.Left - 1f, crushBlock.Top + 4f + (index * 8));
            ...
            else if (crushBlock.crushDir == Vector2.UnitX)
            {
                ...
                for (int index = 0; index < (double)crushBlock.Height / 8.0; ++index)
                {
                    Vector2 point = new(crushBlock.Right + 1f, crushBlock.Top + 4f + (index * 8));
                    if (!crushBlock.Scene.CollideCheck<Water>(point) && crushBlock.Scene.CollideCheck<Solid>(point))
                    {
            ...
            else if (crushBlock.crushDir == -Vector2.UnitY)
            {
                ...
                for (int index = 0; index < (double)crushBlock.Width / 8.0; ++index)
                {
                    Vector2 point = new(crushBlock.Left + 4f + (index * 8), crushBlock.Top - 1f);
                    if (!crushBlock.Scene.CollideCheck<Water>(point) && crushBlock.Scene.CollideCheck<Solid>(point))
                    {
            ...
            else if (crushBlock.crushDir == Vector2.UnitY)
            {
                ...
                for (int index = 0; index < (double)crushBlock.Width / 8.0; ++index)
                {
                    Vector2 point = new(crushBlock.Left + 4f + (index * 8), crushBlock.Bottom + 1f);
                    if (!crushBlock.Scene.CollideCheck<Water>(point) && crushBlock.Scene.CollideCheck<Solid>(point))
                    {
                        crushBlock.SceneAs<Level>().ParticlesFG.Emit(CrushBlock.P_Impact, point + vector2, -1.57079637f);
                        crushBlock.SceneAs<Level>().ParticlesFG.Emit(CrushBlock.P_Impact, point - vector2, -1.57079637f);
            ...
            _ = Audio.Play("event:/game/06_reflection/crushblock_impact", crushBlock.Center);
            ...
            _ = crushBlock.currentMoveLoopSfx.Param("end", 1f);
            crushBlock.currentMoveLoopSfx = null;
            _ = Alarm.Set(crushBlock, 0.5f, sfx.RemoveSelf);
            crushBlock.crushDir = Vector2.Zero;
            crushBlock.TurnOffImages();
            if (!crushBlock.chillOut)
            {
                crushBlock.face.Play("hurt");
                _ = crushBlock.returnLoopSfx.Play("event:/game/06_reflection/crushblock_return_loop");
                yield return 0.4f;
                speed = 0.0f;
                ...
                while (crushBlock.returnStack.Count > 0)
                {
                    ...
                    if ((moveState.Direction.X == 0.0 || crushBlock.ExactPosition.X == (double)moveState.From.X ? (moveState.Direction.Y == 0.0 ? 1 : (crushBlock.ExactPosition.Y == (double)moveState.From.Y ? 1 : 0)) : 0) != 0)
                    {
                        ...
                        if (crushBlock.returnStack.Count <= 0)
                        {
                            ...
                            _ = crushBlock.returnLoopSfx.Stop();
                            if ((double)waypointSfxDelay <= 0.0)
                            {
                                _ = Audio.Play("event:/game/06_reflection/crushblock_rest", crushBlock.Center);
                            }
                        }
                        else if ((double)waypointSfxDelay <= 0.0)
                        {
                            _ = Audio.Play("event:/game/06_reflection/crushblock_rest_waypoint", crushBlock.Center);
                        }
        ...
        private bool MoveHCheck(float amount)
        {
            ...
            if (((double)amount < 0.0 && (double)Left <= level.Bounds.Left) || ((double)amount > 0.0 && (double)Right >= level.Bounds.Right))
            {
                return true;
            }
        ...
        private bool MoveVCheck(float amount)
        {
            ...
            if ((double)amount < 0.0 && (double)Top <= level.Bounds.Top)
            {
                return true;
            }
