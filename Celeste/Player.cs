    ...
    public class Player : Actor
    {
            ...
            Sprite.OnFrameChange = (anim) =>
            {
                if (Scene != null && !Dead)
                {
                        ...
                        (anim.Equals("carryTheoWalk") && (frame == 0 || frame == 6)))
                    {
                        ...
                        if (landed != null)
                            Play(SFX.char_mad_footstep, SurfaceIndex.Param, landed.GetStepSoundIndex(this));
                    }
                    // climbing (holds)
                        ...
                        (anim.Equals(PlayerSprite.ClimbDown) && (frame == 5)))
                    {
                        ...
                        if (holding != null)
                            Play(SFX.char_mad_handhold, SurfaceIndex.Param, holding.GetWallSoundIndex(this, (int)Facing));
                    }
                    else if (anim.Equals("wakeUp") && frame == 19)
                        Play(SFX.char_mad_campfire_stand);
                    else if (anim.Equals("sitDown") && frame == 12)
                        Play(SFX.char_mad_summit_sit);
                ...
                if (Scene != null && !Dead && Sprite.CurrentAnimationID == "idle" && !level.InCutscene && idleTimer > 3f)
                {
                    if (Calc.Random.Chance(0.2f))
                    {
                        ...
                        if (!string.IsNullOrEmpty(next))
                        {
                            ...
                            {
                                if (next == "idleB")
                                    idleSfx = Play(SFX.char_mad_idle_scratch);
                                else if (next == "idleC")
                                    idleSfx = Play(SFX.char_mad_idle_sneeze);
                            }
                            else if (next == "idleA")
                                idleSfx = Play(SFX.char_mad_idle_crackknuckles);
                        }
                    }
                }
            };
        ...
        public override void Update()
        {
            ...
            // wall slide SFX
            {
                ...
                if (isSliding)
                {
                    if (!wallSlideSfx.Playing)
                        Loop(wallSlideSfx, SFX.char_mad_wallslide);
            ...
            if (wasDucking != Ducking)
            {
                wasDucking = Ducking;
                if (wasDucking)
                    Play(SFX.char_mad_duck);
                else if (onGround)
                    Play(SFX.char_mad_stand);
            }
            ...
            if (Speed.X != 0 && ((StateMachine.State == StSwim && !SwimUnderwaterCheck()) || (StateMachine.State == StNormal && CollideCheck<Water>(Position))))
            {
                if (!swimSurfaceLoopSfx.Playing)
                    swimSurfaceLoopSfx.Play(SFX.char_mad_water_move_shallow);
            }
            else
        ...
        public void Jump(bool particles = true, bool playSfx = true)
        {
            ...
            {
                if (launched)
                    Play(SFX.char_mad_jump_assisted);

                if (dreamJump)
                    Play(SFX.char_mad_jump_dreamblock);
                else
                    Play(SFX.char_mad_jump);
            }
        ...
        private void SuperJump()
        {
            ...
            if (Ducking)
            {
                ...
                Play(SFX.char_mad_jump_superslide);
            }
            else
                Play(SFX.char_mad_jump_super);
        ...
        private void WallJump(int dir)
        {
            ...
            if (pushOff != null)
                Play(SFX.char_mad_land, SurfaceIndex.Param, pushOff.GetWallSoundIndex(this, -dir));

            // jump sfx
            Play(dir < 0 ? SFX.char_mad_jump_wall_right : SFX.char_mad_jump_wall_left);
            Sprite.Scale = new Vector2(.6f, 1.4f);
        ...
        private void SuperWallJump(int dir)
        {
            ...
            Play(dir < 0 ? SFX.char_mad_jump_wall_right : SFX.char_mad_jump_wall_left);
            Play(SFX.char_mad_jump_superwall);
            Sprite.Scale = new Vector2(.6f, 1.4f);
        ...
        private void ClimbJump()
        {
            ...
            {
                Play(SFX.char_mad_jump_climb_right);
                Dust.Burst(Center + Vector2.UnitX * 2, Calc.UpLeft, 4);
            }
            else
            {
                Play(SFX.char_mad_jump_climb_left);
                Dust.Burst(Center + Vector2.UnitX * -2, Calc.UpRight, 4);
            }
        }
        ...
        private void Throw()
        {
            if (Holding != null)
            {
                ...
                Play(SFX.char_mad_crystaltheo_throw);
                Sprite.Play("throw");
                Holding = null;
            }
        }
        ...
        private void OnCollideH(CollisionData data)
        {
            if (StateMachine.State == StStarFly)
            {
                ...
                {
                    Play(SFX.game_06_feather_state_bump);
                    Input.Rumble(RumbleStrength.Light, RumbleLength.Medium);
            ...
            if (Speed.Y > 0)
            {
                ...
                if (StateMachine.State != StClimb)
                {
                    ...
                    if (platform != null)
                    {
                        ...
                        if (surface >= 0)
                            Play(playFootstepOnLand > 0f ? SFX.char_mad_footstep : SFX.char_mad_land, SurfaceIndex.Param, surface);
        ...
        private void ClimbBegin()
        {
            ...
            if (platform != null)
                Play(SFX.char_mad_grab, SurfaceIndex.Param, platform.GetWallSoundIndex(this, (int)Facing));
        }
        ...
        private int ClimbUpdate()
        {
            ...
            if (climbNoMoveTimer <= 0 && booster != null)
            {
                ...
                if (conveyorLoopSfx == null)
                    conveyorLoopSfx = Audio.Play(SFX.game_09_conveyor_activate, Position, "end", 0);
            ...
            if (!calledDashEvents)
            {
                ...
                if (CurrentBooster == null)
                {
                    ...
                    // SFX
                    {
                        ...
                        {
                            if (wasDashB)
                                Play(SFX.char_mad_dash_pink_right);
                            else
                                Play(SFX.char_mad_dash_red_right);
                        }
                        else
                        {
                            if (wasDashB)
                                Play(SFX.char_mad_dash_pink_left);
                            else
                                Play(SFX.char_mad_dash_red_left);
                        }

                        if (SwimCheck())
                            Play(SFX.char_mad_water_dash_gen);
                    }
        ...
        {
            Play(SFX.char_mad_crystaltheo_lift);
            Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
        ...
        private void DreamDashBegin()
        {
            ...
            Play(SFX.char_mad_dreamblock_enter);
            Loop(dreamSfxLoop, SFX.char_mad_dreamblock_travel);
        }
        ...
        private void DreamDashEnd()
        {
            ...
            Stop(dreamSfxLoop);
            Play(SFX.char_mad_dreamblock_exit);
            Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
        }
            ...
            if (block == null)
            {
                if (DreamDashedIntoSolid())
                {
                    if (SaveData.Instance.AssistMode && SaveData.Instance.Assists.Invincible)
                    {
                        Position = oldPos;
                        Speed *= -1;
                        Play(SFX.game_assist_dreamblockbounce);
                    }
                    else
                        Die(Vector2.Zero);
                }
        ...
        private void StarFlyBegin()
        {
            ...
            starFlyLoopSfx.Play(SFX.game_06_feather_state_loop, "feather_speed", 1);
            starFlyWarningSfx.Stop();
        }

        private void StarFlyEnd()
        {
            Play(SFX.game_06_feather_state_end);
        ...
        private IEnumerator StarFlyCoroutine()
        {
            ...
            starFlyWarningSfx.Play(SFX.game_06_feather_state_warning);
        }
        ...
        private IEnumerator TempleFallCoroutine()
        {
            ...
            Play(SFX.char_mad_mirrortemple_landing);
            if (Dashes <= 1)
        ...
        private IEnumerator ReflectionFallCoroutine()
        {
            ...
            level.Session.Audio.Music.Event = SFX.music_reflection_main;
        ...
        private IEnumerator IntroJumpCoroutine()
        {
            ...
            // slow down
            {
                ...
                {
                    yield return 0.2f;
                    Play(SFX.char_mad_summit_areastart);
