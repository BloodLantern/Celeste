...
namespace Celeste
{
    ...
    public class Player : Actor
    {
        ...
        private readonly Monocle.Sprite sweatSprite;
        ...
        private readonly List<Player.ChaserStateSound> activeSounds = new();
            ...
            : base(new Vector2((int)position.X, (int)position.Y))
        {
            ...
            ChaserStates = new List<Player.ChaserState>();
            ...
            Sprite.OnFrameChange = anim =>
            {
                ...
                if ((anim.Equals("runSlow_carry") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("runFast") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("runSlow") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("walk") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("runStumble") && currentAnimationFrame == 6) || (anim.Equals("flip") && currentAnimationFrame == 4) || (anim.Equals("runWind") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("idleC") && Sprite.Mode == PlayerSpriteMode.MadelineNoBackpack && (currentAnimationFrame == 3 || currentAnimationFrame == 6 || currentAnimationFrame == 8 || currentAnimationFrame == 11)) || (anim.Equals("carryTheoWalk") && (currentAnimationFrame == 0 || currentAnimationFrame == 6)) || (anim.Equals("push") && (currentAnimationFrame == 8 || currentAnimationFrame == 15)))
                {
                    ...
                    {
                        _ = Play("event:/char/madeline/footstep", "surface_index", platformByPriority.GetStepSoundIndex(this));
                    }
                }
                else if ((anim.Equals("climbUp") && currentAnimationFrame == 5) || (anim.Equals("climbDown") && currentAnimationFrame == 5))
                {
                    ...
                    if (platformByPriority != null)
                    {
                        _ = Play("event:/char/madeline/handhold", "surface_index", platformByPriority.GetWallSoundIndex(this, (int)Facing));
                    }
                }
                else if (anim.Equals("wakeUp") && currentAnimationFrame == 19)
                {
                    _ = Play("event:/char/madeline/campfire_stand");
                }
                else if (anim.Equals("sitDown") && currentAnimationFrame == 12)
                {
                    _ = Play("event:/char/madeline/summit_sit");
                }
        ...
        {
            nextSpriteMode = new PlayerSpriteMode?(mode);
        }
        ...
        public override void Added(Scene scene)
        {
            ...
            else if ((double)X > level.Bounds.Center.X && IntroType != Player.IntroTypes.None)
            {
                Facing = Facings.Left;
            }
        ...
        {
            if (SaveData.Instance.Assists.InvisibleMotion && InControl && ((!LoseShards && StateMachine.State != 1 && StateMachine.State != 3) || (double)Speed.LengthSquared() > 800.0))
            {
                return;
            }
            ...
            else
            {
                ...
                {
                    Sprite.Color = IsTired && flash ? Color.Red : Color.White;
                }
        ...
        public override void Update()
        {
            ...
            highestAirY = !LoseShards ? Math.Min(Y, highestAirY) : Y;
            ...
            if (LoseShards && StateMachine.State != 1)
            {
                AutoJump = false;
                Stamina = 110f;
                wallSlideTimer = 1.2f;
            ...
            else if (!Inventory.NoRefills)
            {
                ...
                else if (LoseShards && (CollideCheck<Solid, NegaBlock>(Position + Vector2.UnitY) || CollideCheckOutside<JumpThru>(Position + Vector2.UnitY)) && (!CollideCheck<Spikes>(Position) || SaveData.Instance.Assists.Invincible))
                {
                    _ = RefillDash();
                }
            }
            ...
            else if (climbHopSolid != null && climbHopSolid.Position != climbHopSolidPosition)
            {
                ...
                _ = MoveHExact((int)vector2.X);
                _ = MoveVExact((int)vector2.Y);
            }
            if (noWindTimer > 0.0)
            {
            ...
            if (launched)
            {
                if ((double)Speed.LengthSquared() < 140 * 140)
                {
                    launched = false;
                }
                else
            ...
            if (!LoseShards && Speed.Y <= 0.0 && (StateMachine.State != 1 || lastClimbMove == -1) && CollideCheck<JumpThru>() && !JumpThruBoostBlockedCheck())
            {
                _ = MoveV(-40f * Engine.DeltaTime);
            }

            if (!LoseShards && DashAttacking && DashDir.Y == 0.0 && (CollideCheck<Solid>(Position + (Vector2.UnitY * 3f)) || CollideCheckOutside<JumpThru>(Position + (Vector2.UnitY * 3f))) && !DashCorrectCheck(Vector2.UnitY * 3f))
            {
                _ = MoveVExact(3);
            }

            if (Speed.Y > 0.0 && CanUnDuck && Collider != starFlyHitbox && !LoseShards && jumpGraceTimer <= 0.0)
            {
                Ducking = false;
            }
            ...
            if ((Sprite.CurrentAnimationID == null || !Sprite.CurrentAnimationID.Equals("wallslide") ? 0 : (Speed.Y > 0.0 ? 1 : 0)) != 0)
            {
                ...
                if (platformByPriority != null)
                {
                    _ = wallSlideSfx.Param("surface_index", platformByPriority.GetWallSoundIndex(this, (int)Facing));
                }
            }
            else
            {
            ...
            if (StateMachine.State != 18)
            {
                foreach (Trigger entity in Scene.Tracker.GetEntities<Trigger>())
                {
                    ...
                    {
                        _ = triggersInside.Remove(entity);
            ...
            if (InControl || ForceCameraUpdate)
            {
                ...
                else
                {
                    ...
                    level.Camera.Position = position + ((cameraTarget - position) * (1f - (float) Math.Pow(0.001f / num, Engine.DeltaTime)));
                }
            }
            ...
            if (wasDucking != Ducking)
            {
                ...
                if (wasDucking)
                {
                    _ = Play("event:/char/madeline/duck");
                }
                else if (LoseShards)
                {
                    _ = Play("event:/char/madeline/stand");
                }
            }
            if (Speed.X != 0.0 && ((StateMachine.State == 3 && !SwimUnderwaterCheck()) || (StateMachine.State == 0 && CollideCheck<Water>(Position))))
            {
                ...
                {
                    _ = swimSurfaceLoopSfx.Play("event:/char/madeline/water_move_shallow");
                }
            }
            else
            {
                _ = swimSurfaceLoopSfx.Stop();
            }

            wasOnGround = LoseShards;
            windMovedUp = false;
        }
        ...
        private void CreateTrail()
        {
            ...
            {
                TrailManager.Add(this, scale, wasDashB ? Player.NormalHairColor : Player.UsedHairColor);
            }
        }
        ...
        private void UpdateChaserStates()
        {
            ...
            ChaserStates.Add(new Player.ChaserState(this));
            activeSounds.Clear();
        }
        ...
        public void UpdateHair(bool applyGravity)
        {
            ...
            {
                Hair.Color = Sprite.Mode != PlayerSpriteMode.MadelineAsBadeline ? Color.Lerp(Hair.Color, Player.UsedHairColor, 6f * Engine.DeltaTime) : Color.Lerp(Hair.Color, Player.UsedBadelineHairColor, 6f * Engine.DeltaTime);
            }
            else
            {
                Color color;
                ...
                {
                    color = Sprite.Mode != PlayerSpriteMode.MadelineAsBadeline ? (Dashes != 2 ? Player.NormalHairColor : Player.TwoDashesHairColor) : (Dashes != 2 ? Player.NormalBadelineHairColor : Player.TwoDashesBadelineHairColor);
                }
        ...
        private void UpdateSprite()
        {
            ...
            if (InControl && Sprite.CurrentAnimationID != "throw" && StateMachine.State != 20 && StateMachine.State != 18 && StateMachine.State != 19 && StateMachine.State != 21)
            {
                ...
                else if (Sprite.CurrentAnimationID != "dreamDashOut")
                {
                    if (DashAttacking)
                    {
                        if (LoseShards && DashDir.Y == 0.0 && !Ducking && Speed.X != 0.0 && moveX == -Math.Sign(Speed.X))
                        {
            ...
            Sprite.Rate = level.InSpace ? 0.5f : 1f;
        }
        ...
        public bool GetChasePosition(float sceneTime, float timeAgo, out Player.ChaserState chaseState)
        {
            ...
            {
                bool flag = false;
                foreach (Player.ChaserState chaserState in ChaserStates)
                {
        ...
        public bool CanRetry => StateMachine.State switch
        {
            12 or 13 or 14 or 15 or 18 or 25 => false,
            _ => true,
        };

        public bool TimePaused => Dead
|| StateMachine.State switch
{
    ...
    _ => false,
        ...
        public bool LoseShards { get; private set; }
        ...
        public void HiccupJump()
        {
            ...
            _ = Play(Ducking ? "event:/new_content/char/madeline/hiccup_ducking" : "event:/new_content/char/madeline/hiccup_standing");
        }
        ...
        public void Jump(bool particles = true, bool playSfx = true)
        {
            ...
            _ = LaunchedBoostCheck();
            if (playSfx)
            {
                if (launched)
                {
                    _ = Play("event:/char/madeline/jump_assisted");
                }

                _ = dreamJump ? Play("event:/char/madeline/jump_dreamblock") : Play("event:/char/madeline/jump");
            }
        ...
        private void SuperJump()
        {
            ...
            _ = Play("event:/char/madeline/jump");
            if (Ducking)
            {
                Ducking = false;
                Speed.X *= 1.25f;
                Speed.Y *= 0.5f;
                _ = Play("event:/char/madeline/jump_superslide");
                gliderBoostDir = Calc.AngleToVector(-3f * (float)Math.PI / 16f, 1f);
            }
            else
            {
                gliderBoostDir = Calc.AngleToVector(-0.7853982f, 1f);
                _ = Play("event:/char/madeline/jump_super");
            }
            varJumpSpeed = Speed.Y;
            launched = true;
            Sprite.Scale = new Vector2(0.6f, 1.4f);
        ...
        private void WallJump(int dir)
        {
            ...
            if (platformByPriority != null)
            {
                ...
                _ = Play("event:/char/madeline/landing", "surface_index", index);
                if (platformByPriority is DreamBlock)
                {
                    (platformByPriority as DreamBlock).FootstepRipple(Position + new Vector2(dir * 3, -4f));
                }
            }
            _ = Play(dir < 0 ? "event:/char/madeline/jump_wall_right" : "event:/char/madeline/jump_wall_left");
            Sprite.Scale = new Vector2(0.6f, 1.4f);
            if (dir == -1)
            {
        ...
        private void SuperWallJump(int dir)
        {
            ...
            launched = true;
            _ = Play(dir < 0 ? "event:/char/madeline/jump_wall_right" : "event:/char/madeline/jump_wall_left");
            _ = Play("event:/char/madeline/jump_superwall");
            Sprite.Scale = new Vector2(0.6f, 1.4f);
            int index = -1;
            Platform platformByPriority = SurfaceIndex.GetPlatformByPriority(CollideAll<Platform>(Position - (Vector2.UnitX * dir * 4f), temp));
            if (platformByPriority != null)
        ...
        private void ClimbJump()
        {
            ...
            {
                _ = Play("event:/char/madeline/jump_climb_right");
                Dust.Burst(Center + (Vector2.UnitX * 2f), -2.3561945f, 4, DustParticleFromSurfaceIndex(index));
            }
            else
            {
                _ = Play("event:/char/madeline/jump_climb_left");
                Dust.Burst(Center + (Vector2.UnitX * -2f), -0.7853982f, 4, DustParticleFromSurfaceIndex(index));
            }
        }
        ...
        public void Bounce(float fromY)
        {
            ...
            Collider collider = Collider;
            Collider = normalHitbox;
            _ = MoveVExact((int)((double)fromY - (double)Bottom));
        ...
        public bool SideBounce(int dir, float fromX, float fromY)
        {
            ...
            Collider collider = Collider;
            Collider = normalHitbox;
            _ = MoveV(Calc.Clamp(fromY - Bottom, -4f, 4f));
            if (dir > 0)
            {
                _ = MoveH(fromX - Left);
            }
        ...
        public void Throw()
        {
            ...
            else
            {
                ...
                _ = Play("event:/char/madeline/crystaltheo_throw");
                Sprite.Play("throw");
            }
            Holding = null;
        }
        ...
        {
            return StateMachine.State != 23
&& (StateMachine.State == 9
                ? CollideCheck(solid)
                : StateMachine.State is 1 or 6
                ? CollideCheck(solid, Position + (Vector2.UnitX * (float)Facing))
                : climbTriggerDir != 0 ? CollideCheck(solid, Position + (Vector2.UnitX * climbTriggerDir)) : base.IsRiding(solid));
        }

        public override bool IsRiding(JumpThru jumpThru)
        {
            return StateMachine.State != 9 && StateMachine.State != 1 && Speed.Y >= 0.0 && base.IsRiding(jumpThru);
        }

        public bool BounceCheck(float y)
        {
            return (double)Bottom <= (double)y + 3.0;
        }
        ...
        public void PointBounce(Vector2 from)
        {
            ...
            Speed.X = Speed.X == 0.0 ? -(int)Facing * 100f : Math.Sign(Speed.X) * 100f;
        }
        ...
        private void OnCollideH(CollisionData data)
        {
            ...
            if (StateMachine.State == StStarFly)
            {
                ...
                {
                    _ = Play("event:/game/06_reflection/feather_state_bump");
                    Input.Rumble(RumbleStrength.Light, RumbleLength.Medium);
                    Speed.X *= -0.5f;
                }
            }
            else
            {
                ...
                if (StateMachine.State is StDash or StRedDash)
                {
                    if (LoseShards && DuckFreeAt(Position + (Vector2.UnitX * Math.Sign(Speed.X))))
                    {
                        Ducking = true;
                        return;
                    }
                    if (Speed.Y == 0.0 && Speed.X != 0.0)
                    {
                        for (int index1 = 1; index1 <= 4; ++index1)
                        {
                            for (int index2 = 1; index2 >= -1; index2 -= 2)
                            {
                                ...
                                {
                                    _ = MoveVExact(index1 * index2);
                                    _ = MoveHExact(Math.Sign(Speed.X));
                                    return;
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    _ = level.Displacement.AddBurst(Center, 0.5f, 8f, 48f, 0.4f, Ease.QuadOut, Ease.QuadOut);
        ...
        private void OnCollideV(CollisionData data)
        {
            ...
            else
            {
                ...
                if (data.Hit != null && data.Hit.OnDashCollide != null)
                {
                    ...
                    {
                        int num;
                        _ = (int)data.Hit.OnDashCollide(this, data.Direction);
                        return;
                    }
                }
                if (Speed.Y > 0.0)
                {
                    ...
                    if (StateMachine.State != 1)
                    {
                        ...
                        if (platformByPriority != null)
                        {
                            ...
                            {
                                _ = Play(playFootstepOnLand > 0.0 ? "event:/char/madeline/footstep" : "event:/char/madeline/landing", "surface_index", index);
                            }
        ...
        protected override void OnSquish(CollisionData data)
        {
            ...
            if (!TrySquishWiggle(data, wiggleY: 5))
            {
                ...
                _ = Die(Vector2.Zero, evenIfInvincible);
            }
            else
            {
                if (!flag || !CanUnDuck)
        ...
        {
            return (double)Left + (dir * 2) >= level.Bounds.Left && (double)Right + (dir * 2) < level.Bounds.Right;
        }
        ...
        {
            return ClimbBoundsCheck(dir) && !ClimbBlocker.Check(Scene, this, Position + (Vector2.UnitY * yAdd) + (Vector2.UnitX * 2f * (float)Facing)) && CollideCheck<Solid>(Position + new Vector2(dir * 2, yAdd));
        }

        private int NormalUpdate()
        {
            if (LiftBoost.Y < 0.0 && wasOnGround && !LoseShards && Speed.Y >= 0.0)
            {
                Speed.Y = LiftBoost.Y;
            }
            ...
            if (Holding == null)
            {
                ...
                if (Ducking)
                {
                    if (LoseShards && (double)(float)Input.MoveY != 1.0)
                    {
                        if (CanUnDuck)
                        {
                            Ducking = false;
                ...
                else if (LoseShards && (double)(float)Input.MoveY == 1.0 && Speed.Y >= 0.0)
                {
                    Ducking = true;
            ...
            else
            {
                ...
                if (!Ducking && LoseShards && (double)(float)Input.MoveY == 1.0 && Speed.Y >= 0.0 && !holdCannotDuck)
                {
                    Drop();
                    Ducking = true;
                    Sprite.Scale = new Vector2(1.4f, 0.6f);
                }
                else if (LoseShards && Ducking && Speed.Y >= 0.0)
                {
                    if (CanUnDuck)
                    {
                        Ducking = false;
                ...
                else if (LoseShards && (double)(float)Input.MoveY != 1.0 && holdCannotDuck)
                {
            ...
            {
                float num1 = LoseShards ? 1f : 0.65f;
                if (LoseShards && level.CoreMode == Session.CoreModes.Cold)
                {
                    num1 *= 0.3f;
                }
                ...
                else if (Holding != null && Holding.SlowFall && !LoseShards)
                {
                    num2 = 108.000008f;
                    num1 *= 0.5f;
                }
        ...
        private void ClimbBegin()
        {
            ...
            _ = Play("event:/char/madeline/grab", "surface_index", platformByPriority.GetWallSoundIndex(this, (int)Facing));
        ...
        private void ClimbEnd()
        {
            ...
            {
                _ = (int)conveyorLoopSfx.setParameterValue("end", 1f);
                _ = (int)conveyorLoopSfx.release();
        ...
        private int ClimbUpdate()
        {
            ...
            {
                Speed += LiftBoost;
                _ = Play("event:/char/madeline/grab_letgo");
                return 0;
            }
        ...
        private void ClimbHop()
        {
            ...
            fastJump = false;
            noWindTimer = 0.3f;
            _ = Play("event:/char/madeline/climb_ledge");
        }
        ...
        public bool CanDash => (Input.CrouchDashPressed || Input.DashPressed) && dashCooldownTimer <= 0.0 && Dashes > 0 && (TalkComponent.PlayerOver == null || !Input.Talk.Pressed)
&& (LastBooster == null || !LastBooster.Ch9HubTransition || !LastBooster.BoostingPlayer);
        ...
        private void CallDashEvents()
        {
            ...
            if (CurrentBooster == null)
            {
                ...
                _ = flag
                    ? wasDashB ? Play("event:/char/madeline/dash_pink_right") : Play("event:/char/madeline/dash_red_right")
                    : wasDashB ? Play("event:/char/madeline/dash_pink_left") : Play("event:/char/madeline/dash_red_left");

                if (SwimCheck())
                {
                    _ = Play("event:/char/madeline/water_dash_gen");
                }
        ...
        private void DashBegin()
        {
            ...
            dashStartedOnGround = LoseShards;
            launched = false;
            canCurveDash = true;
            ...
            if (!LoseShards && Ducking && CanUnDuck)
            {
                Ducking = false;
            }
        ...
        private int DashUpdate()
        {
            ...
            if ((double)Math.Abs(DashDir.Y) < 0.10000000149011612)
            {
                foreach (JumpThru entity in Scene.Tracker.GetEntities<JumpThru>())
                {
                    ...
                    {
                        _ = MoveVExact((int)((double)entity.Top - (double)Bottom));
                    }
                }
        ...
        private IEnumerator DashCoroutine()
        {
            ...
            _ = player.level.Displacement.AddBurst(player.Center, 0.4f, 8f, 64f, 0.5f, Ease.QuadOut, Ease.QuadOut);
            ...
            if (player.LoseShards && player.DashDir.X != 0.0 && player.DashDir.Y > 0.0 && player.Speed.Y > 0.0 && (!player.Inventory.DreamDash || !player.CollideCheck<DreamBlock>(player.Position + Vector2.UnitY)))
            {
            ...
            _ = SlashFx.Burst(player.Center, player.DashDir.Angle());
            player.CreateTrail();
            if (SaveData.Instance.Assists.SuperDashing)
            {
            ...
            yield return SaveData.Instance.Assists.SuperDashing ? 0.3f : (object)0.15f;
            player.CreateTrail();
        ...
        {
            return CollideCheck<Water>(Position + (Vector2.UnitY * -8f)) && CollideCheck<Water>(Position);
        }
        ...
        {
            return CollideCheck<Water>(Position) && CollideCheck<Water>(Position + (Vector2.UnitY * -12f));
        }
        ...
        private void RedDashBegin()
        {
            ...
            dashRefillCooldownTimer = 0.1f;
            StartedDashing = true;
            _ = level.Displacement.AddBurst(Center, 0.5f, 0.0f, 80f, 0.666f, Ease.QuadOut, Ease.QuadOut);
            Input.Rumble(RumbleStrength.Strong, RumbleLength.Medium);
            dashAttackTimer = 0.3f;
            gliderBoostTimer = 0.55f;
            DashDir = Speed = Vector2.Zero;
            if (!LoseShards && CanUnDuck)
            {
                Ducking = false;
            }
        ...
        {
            Player player = this;
            _ = player.Play("event:/char/madeline/crystaltheo_lift");
            Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
            ...
            {
                _ = Audio.Play("event:/new_content/game/10_farewell/glider_platform_dissipate", player.Position);
            }
            ...
            if (player.Holding != null && player.Holding.SlowFall)
            {
                ...
                if (player.LoseShards && (double)(float)Input.MoveY == 1.0)
                {
        ...
        private void DreamDashBegin()
        {
            ...
            Stamina = 110f;
            dreamJump = false;
            _ = Play("event:/char/madeline/dreamblock_enter");
        ...
        private void DreamDashEnd()
        {
            ...
            Stop(dreamSfxLoop);
            _ = Play("event:/char/madeline/dreamblock_exit");
            Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
        }
            ...
            if (dreamBlock == null)
            {
                if (DreamDashedIntoSolid())
                {
                    if (SaveData.Instance.Assists.Invincible)
                    {
                        Position = position;
                        Speed *= -1f;
                        _ = Play("event:/game/general/assist_dreamblockbounce");
                    }
                    else
                    {
                        _ = Die(Vector2.Zero);
        ...
        private void StarFlyBegin()
        {
            ...
            _ = starFlyLoopSfx.Play("event:/game/06_reflection/feather_state_loop", "feather_speed", 1f);
            _ = starFlyWarningSfx.Stop();
        }

        private void StarFlyEnd()
        {
            _ = Play("event:/game/06_reflection/feather_state_end");
            _ = starFlyWarningSfx.Stop();
            _ = starFlyLoopSfx.Stop();
            Hair.DrawPlayerSpriteOutline = false;
            Sprite.Color = Color.White;
            _ = level.Displacement.AddBurst(Center, 0.25f, 8f, 32f);
        ...
        private IEnumerator StarFlyCoroutine()
        {
            ...
            _ = player.level.Displacement.AddBurst(player.Center, 0.25f, 8f, 32f);
            ...
            _ = player.starFlyWarningSfx.Play("event:/game/06_reflection/feather_state_warning");
        }
        ...
        private int StarFlyUpdate()
        {
            ...
            else
            {
                ...
                _ = starFlyLoopSfx.Param("feather_speed", flag1 ? 0.0f : 1f);
                ...
                {
                    Sprite.Color = Sprite.Color == starFlyColor ? Player.NormalHairColor : starFlyColor;
                }
            }
            return 19;
        }
        ...
        private IEnumerator TempleFallCoroutine()
        {
            Player player = this;
            player.Sprite.Play("fallFast");
            while (!player.LoseShards)
            {
                yield return null;
            }

            _ = player.Play("event:/char/madeline/mirrortemple_big_landing");
            if (player.Dashes <= 1)
            {
        ...
        private IEnumerator IntroJumpCoroutine()
        {
            ...
            {
                yield return 0.2f;
                _ = player.Play("event:/char/madeline/summit_areastart");
        ...
        public IEnumerator MoonLanding(Vector2 groundPosition)
        {
            ...
            {
                _ = player.MoveV(-200f * Engine.DeltaTime);
                yield return null;
            }
            player.Speed.Y = -200f;
            while (player.Speed.Y < 0.0)
        ...
        {
            _ = Play("event:/char/madeline/revive");
            Depth = -1000000;
            introEase = 1f;
        ...
        public IEnumerator IntroThinkForABitCoroutine()
        {
            ...
            {
                _ = player.MoveH(32f * Engine.DeltaTime);
                yield return null;
            }
            player.Sprite.Play("idle");
            yield return 0.3f;
        ...
        {
            DashBegin();
            _ = Play("event:/char/madeline/dash_red_right");
            Sprite.Play("dash");
        }
        ...
        private IEnumerator BirdDashTutorialCoroutine()
        {
            ...
            if (climbing)
            {
                ...
                player.Y = (float)Math.Round((double)player.Y);
                _ = player.Play("event:/char/madeline/climb_ledge");
        ...
        {
            AddChaserStateSound(sound, null, 0.0f, Player.ChaserStateSound.Actions.Loop);
            _ = sfx.Play(sound);
        }
        ...
        public void Stop(SoundSource sfx)
        {
            ...
            AddChaserStateSound(sfx.EventName, null, 0.0f, Player.ChaserStateSound.Actions.Stop);
            _ = sfx.Stop();
        }

        private void AddChaserStateSound(string sound, Player.ChaserStateSound.Actions action)
        {
            AddChaserStateSound(sound, null, 0.0f, action);
        }
        ...
        {
            _ = SFX.MadelineToBadelineSound.TryGetValue(sound, out string str);
            if (str == null)
            {
                return;
            }

            activeSounds.Add(new Player.ChaserStateSound()
            {
                Event = str,
                Parameter = param,
        ...
        public struct ChaserState
        {
            ...
            public ChaserState(Player player)
            {
                ...
                sound0 = Sounds > 0 ? activeSounds[0] : new Player.ChaserStateSound();
                sound1 = Sounds > 1 ? activeSounds[1] : new Player.ChaserStateSound();
                sound2 = Sounds > 2 ? activeSounds[2] : new Player.ChaserStateSound();
                sound3 = Sounds > 3 ? activeSounds[3] : new Player.ChaserStateSound();
                sound4 = Sounds > 4 ? activeSounds[4] : new Player.ChaserStateSound();
            }

            public Player.ChaserStateSound this[int index] => index switch
            {
                0 => sound0,
                1 => sound1,
                2 => sound2,
