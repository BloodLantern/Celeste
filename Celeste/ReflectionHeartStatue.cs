...
namespace Celeste
{
    public class ReflectionHeartStatue : Entity
    {
        ...
        private readonly List<ReflectionHeartStatue.Torch> torches = new();
        ...
        public override void Added(Scene scene)
        {
            ...
            Monocle.Image image1 = new(GFX.Game["objects/reflectionHeart/statue"]);
            _ = image1.JustifyOrigin(0.5f, 1f);
            --image1.Origin.Y;
            Add(image1);
            List<string[]> strArrayList = new()
            {
                ReflectionHeartStatue.Code,
                FlipCode(true, false),
                FlipCode(false, true),
                FlipCode(true, true)
            };
            ...
            {
                ReflectionHeartStatue.Torch torch = new(session, offset + nodes[index], index, strArrayList[index]);
                Scene.Add(torch);
                torches.Add(torch);
            }
            ...
            {
                Monocle.Image image2 = new(GFX.Game["objects/reflectionHeart/gem"]);
            ...
            dashListener.OnDash = dir =>
            {
                ...
                _ = Audio.Play("event:/game/06_reflection/supersecret_dashflavour", entity != null ? entity.Position : Vector2.Zero, "dash_direction", num);
                ...
                foreach (ReflectionHeartStatue.Torch torch in torches)
                {
        ...
        private IEnumerator ActivateRoutine()
        {
            ...
            yield return 0.533f;
            _ = Audio.Play("event:/game/06_reflection/supersecret_heartappear");
            Entity dummy = new(reflectionHeartStatue.Position + new Vector2(0.0f, -52f))
            {
                Depth = 1
            };
            reflectionHeartStatue.Scene.Add(dummy);
            Monocle.Image white = new(GFX.Game["collectables/heartgem/white00"]);
            _ = white.CenterOrigin();
            white.Scale = Vector2.Zero;
            dummy.Add(white);
            BloomPoint glow = new(0.0f, 16f);
            dummy.Add(glow);
            List<Entity> absorbs = new();
            for (int i = 0; i < 20; ++i)
            {
                AbsorbOrb absorbOrb = new(reflectionHeartStatue.Position + new Vector2(0.0f, -20f), dummy);
        ...
        public class Torch : Entity
        {
                ...
                : base(position)
            {
                ...
                Monocle.Image image = new(GFX.Game.GetAtlasSubtextures("objects/reflectionHeart/hint")[index]);
            ...
            public void Activate()
            {
                ...
                {
                    _ = Audio.Play("event:/game/06_reflection/supersecret_torch_" + (Index + 1), Position);
                    PlayLit();
                });
            }
