...
namespace Celeste
{
    public class LevelEnter : Scene
    {
        ...
        public static void Go(Session session, bool fromSaveData)
        {
            ...
            Engine.Scene = flag && session.Area.ID == 0
                ? new IntroVignette(session, snow)
                : flag && session.Area.ID == 7 && session.Area.Mode == AreaMode.Normal
                    ? new SummitVignette(session)
                    : flag && session.Area.ID == 9 && session.Area.Mode == AreaMode.Normal
                                ? new CoreVignette(session, snow)
                                : new LevelEnter(session, fromSaveData);
        }
        ...
        private IEnumerator Routine()
        {
            ...
            {
                yield return 1f;
                levelEnter.Add(levelEnter.postcard = new Postcard(Dialog.Get("postcard_area_" + area), area));
                yield return levelEnter.postcard.DisplayRoutine();
            }
            if (levelEnter.session.StartedFromBeginning && !levelEnter.fromSaveData && levelEnter.session.Area.Mode == AreaMode.BSide)
            {
                LevelEnter.BSideTitle title = new(levelEnter.session);
                levelEnter.Add(title);
                _ = Audio.Play("event:/ui/main/bside_intro_text");
