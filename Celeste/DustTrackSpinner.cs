...
namespace Celeste
{
    public class DustTrackSpinner : TrackSpinner
    {
        ...
        private void Establish()
        {
            ...
            bool flag = Scene.CollideCheck<Solid>(new Rectangle((int) (X + vector2_2.X * 4.0) - 2, (int) (Y + vector2_2.Y * 4.0) - 2, 4, 4));
            if (!flag)
            {
                vector2_2 = -vector2_2;
                flag = Scene.CollideCheck<Solid>(new Rectangle((int) (X + vector2_2.X * 4.0) - 2, (int) (Y + vector2_2.Y * 4.0) - 2, 4, 4));
            }
            if (!flag)
                return;
            ...
            for (int index = 8; index < (double) num & flag; index += 8)
                flag = flag && Scene.CollideCheck<Solid>(new Rectangle((int) (X + vector2_2.X * 4.0 + vector2_1.X * (double) index) - 2, (int) (Y + vector2_2.Y * 4.0 + vector2_1.Y * (double) index) - 2, 4, 4));
            if (!flag)
                return;
            ...
            double startAngle = outwards.Angle();
