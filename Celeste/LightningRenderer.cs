...
namespace Celeste
{
    ...
    public class LightningRenderer : Entity
    {
        ...
        public void StopAmbience() => AmbientSfx.Stop();
        ...
        public override void Render()
        {
            ...
            {
                if (lightning.Visible)
                    Draw.SpriteBatch.Draw((RenderTarget2D) GameplayBuffers.Lightning, lightning.Position, new Rectangle((int) lightning.X, (int) lightning.Y, lightning.VisualWidth, lightning.VisualHeight), Color.White);
            }
            ...
            GFX.DrawVertices(camera.Matrix, edgeVerts, index1);
            ...
            float thickness = 1f)
        {
            ...
            float num4 = 0.0f;
            float num5 = index + (float) (((b - a).Length() / 4.0 + 1.0) * 6.0);
            while (num5 >= (double) verts.Length)
            ...
            do
            {
                ...
                Vector2 vector2_4 = num4 >= (double) num1 ? b : vector2_3 + (num2 * vector2_1 * num6 - vector2_1);
            ...
            Color color)
        {
            ...
            Vector2 start = a;
            int num2 = 1;
            double num3 = LightningRenderer.PseudoRandRange(ref seed, 0.0f, 6.28318548f);
            float num4 = 0.0f;
            do
            {
                num4 += LightningRenderer.PseudoRandRange(ref seed, 10f, 14f);
                Vector2 vector2_2 = a + vec * num4;
                Vector2 vector2_3 = num4 >= (double) num1 ? b : vector2_2 + num2 * vector2_1 * LightningRenderer.PseudoRandRange(ref seed, 0.0f, 6f);
        ...
        public static float PseudoRandRange(ref uint seed, float min, float max) => min + (float) ((LightningRenderer.PseudoRand(ref seed) & 1023U) / 1024.0 * (max - (double) min));
        ...
        private class Bolt
        {
            ...
            private IEnumerator Run()
            {
                ...
                while (true)
                {
                    ...
                    {
                        Vector2 vector2_1 = Calc.Random.Choose(new Vector2(0.0f, Calc.Random.Range(8, height - 16)), new Vector2(Calc.Random.Range(8, width - 16), 0.0f), new Vector2(width, Calc.Random.Range(8, height - 16)), new Vector2(Calc.Random.Range(8, width - 16), height));
