...
namespace Celeste
{
    public class OuiChapterSelectIcon : Entity
    {
        ...
        private readonly Wiggler newWiggle;
        ...
        private readonly MTexture front;
        private readonly MTexture back;
        ...
        public Vector2 HiddenPosition => new(IdlePosition.X, -100f);
        ...
        public OuiChapterSelectIcon(int area, MTexture front, MTexture back)
        {
            ...
            {
                Rotation = (float)((wiggleLeft ? -(double)f : (double)f) * 0.40000000596046448);
                Scale = Vector2.One * (float)(1.0 + ((double)f * 0.5));
            }));
        ...
        public void Select()
        {
            _ = Audio.Play("event:/ui/world_map/icon/flip_right");
            selected = true;
            hidden = false;
        ...
        {
            _ = Audio.Play("event:/ui/world_map/icon/flip_left");
            hidden = false;
            selected = false;
        ...
        private IEnumerator AssistModeUnlockRoutine(Action onComplete)
        {
            ...
            for (p = 0.0f; (double)p < 1.0; p += Engine.DeltaTime * 4f)
            {
                ...
                Scale = new Vector2(1f - p, (float)(1.0 + ((double)p * 0.25)));
                yield return null;
            }
            shake = Vector2.Zero;
            Scale = Vector2.One;
            ...
            {
                spotlightRadius = (float)(128.0 + ((1.0 - (double)Ease.CubeOut(p)) * 128.0));
                spotlightAlpha = Ease.CubeOut(p) * 0.8f;
                yield return null;
            }
            spotlightAlpha = 0.0f;
        ...
        private IEnumerator HighlightUnlockRoutine(Action onComplete)
        {
            ...
            _ = Audio.Play("event:/ui/postgame/unlock_newchapter_icon");
            HideIcon = false;
            wiggler.Start();
            yield return 2f;
        ...
        private void SetSelectedPercent(Vector2 from, float p)
        {
            ...
            SimpleCurve simpleCurve = new(from, end, ((from + end) / 2f) + new Vector2(0.0f, 30f));
            float num = (float)(1.0 + ((double)p < 0.5 ? (double)p * 2.0 : (1.0 - (double)p) * 2.0));
            Scale.X = (float)Math.Cos((double)Ease.SineInOut(p) * 6.2831854820251465) * num;
            Scale.Y = num;
            Position = simpleCurve.GetPoint(Ease.Invert(Ease.CubeInOut)(p));
            Rotation = (float)((double)Ease.UpDown(Ease.SineInOut(p)) * (Math.PI / 180.0) * 15.0) * rotateDir;
            if ((double)p <= 0.0)
            {
                rotateDir = -1;
            }
        ...
        public override void Update()
        {
            ...
            Depth = SaveData.Instance.LastArea.ID == Area ? -50 : -45;
            if (tween == null)
            {
                if (selected)
                {
        ...
        public override void Render()
        {
            ...
            if (!HideIcon)
            {
                Vector2 scale2 = scale1 * ((float)(100.0 + (44.0 * (double)Ease.CubeInOut(sizeEase))) / width);
