...
namespace Celeste
{
    public class Tentacles : Backdrop
    {
        ...
        private readonly Tentacles.Side side;
        private readonly float width;
        private Vector2 origin;
        private Vector2 outwards;
        private readonly float outwardsOffset;
        private readonly VertexPositionColor[] vertices;
        private int vertexCount;
        private readonly Tentacles.Tentacle[] tentacles;
        private readonly int tentacleCount;
        ...
        public Tentacles(Tentacles.Side side, Color color, float outwardsOffset)
        {
            ...
            float num = 0.0f;
            tentacles = new Tentacles.Tentacle[100];
            for (int index = 0; index < tentacles.Length && (double)num < width + 40.0; ++index)
            {
        ...
        public override void Update(Scene scene)
        {
            ...
            if (num1 != 0)
            {
                ...
                if (entity != null)
                {
                    if (side == Tentacles.Side.Right)
                    {
                        num2 = (float)(320.0 - ((double)entity.X - (double)camera.X) - 160.0);
                    }
                    else if (side == Tentacles.Side.Bottom)
                    {
            ...
            for (int index1 = 0; index1 < tentacleCount; ++index1)
            {
                ...
                tentacles[index1].Position += (float)(((double)num3 - tentacles[index1].Position) * (1.0 - Math.Pow(0.5 * (0.5 + (tentacles[index1].Approach * 0.5)), (double)Engine.DeltaTime)));
                Vector2 vector2_4 = (float)(tentacles[index1].Position + (Math.Sin(scene.TimeActive + (tentacles[index1].Offset * 4.0)) * 8.0) + ((double)(origin - vector2_3).Length() * 0.699999988079071)) * outwards;
                ...
                for (int y = 1; y < 10; ++y)
                {
                    ...
                    float num12 = (float)((tentacles[index1].Offset * 3.0) + (y * (0.10000000149011612 + (tentacles[index1].Step * 0.20000000298023224))) + ((double)num5 * y * 0.10000000149011612));
