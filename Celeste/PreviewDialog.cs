...
namespace Celeste
{
    public class PreviewDialog : Scene
    {
        ...
        private Vector2 listScroll = new(64f, 64f);
        private const float scale = 0.6f;
        private string current;
        private readonly List<object> elements = new();
        private Vector2 textboxScroll = new(0.0f, 0.0f);
        private float delay;
        ...
        public PreviewDialog(Language language = null, float listScroll = 64f, float textboxScroll = 0.0f, string dialog = null)
        {
            ...
            Add(new PreviewDialog.Renderer(this));
        }
        ...
        public override void Update()
        {
            ...
            else
            {
                if (current != null)
                {
                    ...
                    textboxScroll.Y -= (float)(Input.Aim.Value.Y * (double)Engine.DeltaTime * (double)ActiveFont.LineHeight * 20.0);
                ...
                else
                {
                    ...
                    listScroll.Y -= (float)(Input.Aim.Value.Y * (double)Engine.DeltaTime * (double)ActiveFont.LineHeight * 20.0);
        ...
        private void SetCurrent(string id)
        {
            ...
            while (true)
            {
                Textbox textbox2 = new(id, language, new Func<IEnumerator>[0]);
                if (textbox2.SkipToPage(page))
                {
                    if (textbox1 != null)
                    {
                        for (int index = textbox1.Start + 1; index <= textbox2.Start && index < textbox1.Nodes.Count; ++index)
                        {
                            ...
                            {
                                elements.Add((node.Silent ? "Silent " : (object)"").ToString() + "Trigger [" + node.Index + "] " + node.Label);
                            }
                        }
                    }
                    Add(textbox2);
            ...
            if (current != null)
            {
                ...
                foreach (object element in elements)
                {
                    if (element is Textbox textbox)
                    {
                        ...
                        {
                            textbox.Render();
                            language.Font.DrawOutline(language.FontFaceSize, "#" + num1.ToString(), textbox.RenderOffset + new Vector2(32f, 64f), Vector2.Zero, Vector2.One * 0.5f, Color.White, 2f, Color.Black);
                            ++num1;
                            num2 += 300;
                        }
                    }
        ...
        {
            return Mouse.X > (double)listScroll.X && Mouse.Y > listScroll.Y + (i * (double)ActiveFont.LineHeight * 0.60000002384185791) && (double)MInput.Mouse.X < 960.0 && Mouse.Y < listScroll.Y + ((i + 1) * (double)ActiveFont.LineHeight * 0.60000002384185791);
        }
