...
namespace Celeste
{
    public class Checkpoint : Entity
    {
        ...
        public Checkpoint(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Attr(nameof (bg)), data.FirstNodeNullable(offset))
        {
        }
        ...
        public override void Update()
        {
            ...
            if (triggered && sprite.CurrentAnimationID == "on")
            {
                ...
                sprite.Color = Color.White * (float) (0.5 + (float) (1.0 + Math.Sin(sine)) / 2f * 0.5) * fade;
            }
            base.Update();
        }
        ...
        {
            triggered = true;
            Add(light = new VertexLight(Color.White, 0.0f, 16, 32));
            Add(bloom = new BloomPoint(0.0f, 16f));
            light.Position = new Vector2(0.0f, -8f);
            bloom.Position = new Vector2(0.0f, -8f);
            flash.Visible = true;
        ...
        private IEnumerator EaseLightsOn()
        {
            ...
            for (float p = 0.0f; p < 1.0; p += Engine.DeltaTime * 0.5f)
            {
                ...
                light.StartRadius = (int) (lightStartRadius + Calc.YoYo(p) * 8.0);
                light.EndRadius = (int) (lightEndRadius + Calc.YoYo(p) * 16.0);
