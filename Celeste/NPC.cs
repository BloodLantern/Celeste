    ...
    public class NPC : Entity
    {
        ...
        public override void Update()
        {
            ...
            if (UpdateLight && Light != null)
            {
                ...
                Light.Alpha = Calc.Approach(Light.Alpha, X <= (double) (bounds.Left - 16) || Y <= (double) (bounds.Top - 16) || X >= (double) (bounds.Right + 16) || Y >= (double) (bounds.Bottom + 16) || Level.Transitioning ? 0.0f : 1f, Engine.DeltaTime * 2f);
            }
        ...
        public void SetupTheoSpriteSounds() => Sprite.OnFrameChange = anim =>
        {
            ...
            if (anim == "walk" && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim == "run" && (currentAnimationFrame == 0 || currentAnimationFrame == 4))
            {
                ...
                if (platformByPriority == null)
                    return;
                Audio.Play("event:/char/madeline/footstep", Center, "surface_index", platformByPriority.GetStepSoundIndex(this));
            }
            ...
            int? side = null)
        {
            ...
            if (spacing.HasValue)
            {
                ...
                float x1 = npc.X;
                nullable1 = side;
                float? nullable2 = nullable1.HasValue ? nullable1.GetValueOrDefault() : new float?();
                float? nullable3 = spacing;
                float? nullable4 = nullable2.HasValue & nullable3.HasValue ? nullable2.GetValueOrDefault() * nullable3.GetValueOrDefault() : new float?();
                float? nullable5;
                if (!nullable4.HasValue)
                {
                    ...
                    nullable5 = x1 + nullable4.GetValueOrDefault();
                nullable3 = nullable5;
                int x2 = (int) nullable3.Value;
                yield return player1.DummyWalkToExact(x2);
            }
            else if (Math.Abs(npc.X - player.X) < 12.0 || Math.Sign(player.X - npc.X) != side.Value)
            {
                Player player2 = player;
                float x3 = npc.X;
                nullable1 = side;
                float? nullable6 = nullable1.HasValue ? nullable1.GetValueOrDefault() * 12 : new float?();
                int x4 = (int) (nullable6.HasValue ? x3 + nullable6.GetValueOrDefault() : new float?()).Value;
            ...
            bool removeAtEnd = false)
        {
            ...
            float speed = 0.0f;
            while (npc.MoveY && npc.Position != target || !npc.MoveY && npc.X != (double) target.X)
            {
