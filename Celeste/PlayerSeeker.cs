...
namespace Celeste
{
    public class PlayerSeeker : Actor
    {
            ...
            : base(data.Position + offset)
        {
            ...
            {
                if (a is not "flipMouth" and not "flipEyes")
                {
                    return;
                }
        ...
        private IEnumerator IntroSequence()
        {
            ...
            yield return 2f;
            _ = playerSeeker.shaker.ShakeFor(0.5f, false);
            ...
            yield return 1f;
            playerSeeker.BreakOutParticles();
            _ = Audio.Play("event:/game/05_mirror_temple/seeker_statue_break", playerSeeker.Position);
            _ = playerSeeker.shaker.ShakeFor(1f, false);
        ...
        private void OnPlayer(Player player)
        {
            ...
            playerDeadBody.DeathAction = new Action(End);
        ...
        public override void Update()
        {
            ...
            if (enabled && sprite.CurrentAnimationID != "hatch")
            {
                ...
                else
                {
                    ...
                    if ((double)vector2.Length() > 0.0 && sprite.CurrentAnimationID == "idle")
                    {
                        _ = scene.Displacement.AddBurst(Position, 0.5f, 8f, 32f);
                        sprite.Play("spotted");
                        _ = Audio.Play("event:/game/05_mirror_temple/seeker_playercontrolstart");
                    }
                    int num1 = Math.Sign((int)facing);
                    int num2 = Math.Sign(speed.X);
                    if (num2 != 0 && num1 != num2 && Math.Sign(Input.Aim.Value.X) == Math.Sign(speed.X) && (double)Math.Abs(speed.X) > 20.0 && sprite.CurrentAnimationID != "flipMouth" && sprite.CurrentAnimationID != "flipEyes")
                    {
                ...
                _ = MoveH(speed.X * Engine.DeltaTime, new Collision(OnCollide));
                _ = MoveV(speed.Y * Engine.DeltaTime, new Collision(OnCollide));
                ...
                if (entity != null)
                {
                    ...
                    else if ((double)val < 100.0 && entity.Sprite.CurrentAnimationID != "wakeUp")
                    {
                        ...
                        entity.Facing = (double)X > (double)entity.X ? Facings.Left : Facings.Right;
                    }
                    if ((double)val < 50.0 && dashTimer <= 0.0)
                    {
        ...
        private void OnCollide(CollisionData data)
        {
            ...
            else
            {
                ...
                if (data.Hit is not TempleCrackedBlock)
                {
                    return;
                }
        ...
        private void Dash(Vector2 dir)
        {
            ...
            sprite.Scale = dashDirection.X == 0.0 ? new Vector2(0.6f, 1.4f) : new Vector2(1.4f, 0.6f);
        }
        ...
        {
            if (SaveData.Instance.Assists.InvisibleMotion && enabled && (double)speed.LengthSquared() > 100.0)
            {
                return;
            }
