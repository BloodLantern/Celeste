    ...
    public class NorthernLights : Backdrop
    {
        ...
        public NorthernLights()
        {
            ...
            for (int index = 0; index < particles.Length; ++index)
            {
                ...
                particles[index].Color = Calc.Random.Choose(NorthernLights.colors);
            }
        ...
        public override void BeforeRender(Scene scene)
        {
            ...
            foreach (Strand strand in strands)
            {
                Node node1 = strand.Nodes[0];
                for (int index = 1; index < strand.Nodes.Count; ++index)
                {
                    Node node2 = strand.Nodes[index];
            ...
            bool clear = false;
            GaussianBlur.Blur((RenderTarget2D) buffer, GameplayBuffers.TempA, buffer, clear: clear, samples: GaussianBlur.Samples.Five, sampleScale: 0.25f, direction: GaussianBlur.Direction.Vertical);
        ...
        public override void Render(Scene scene) => Draw.SpriteBatch.Draw((RenderTarget2D) buffer, Vector2.Zero, Color.White);
        ...
        private class Strand
        {
            ...
            public Strand() => Reset(Calc.Random.NextFloat());
            ...
            public void Reset(float startPercent)
            {
                ...
                Alpha = 0.0f;
                Nodes.Clear();
                Vector2 vector2 = new Vector2(Calc.Random.Range(-40, 60), Calc.Random.Range(40, 90));
                float num = Calc.Random.NextFloat();
                Color color = Calc.Random.Choose(NorthernLights.colors);
                for (int index = 0; index < 40; ++index)
                {
                    Node node = new Node
                    {
                        ...
                        Color = Color.Lerp(color, Calc.Random.Choose(NorthernLights.colors), Calc.Random.Range(0.0f, 0.3f))
                    };
