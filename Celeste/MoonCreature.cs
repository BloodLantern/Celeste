...
namespace Celeste
{
    ...
    {
        private readonly MoonCreature.TrailNode[] trail;
        ...
        public MoonCreature(Vector2 position)
        {
            ...
            CenterColor = Calc.Random.Choose<Color>(Calc.HexToColor("c34fc7"), Calc.HexToColor("4f95c7"), Calc.HexToColor("53c74f"));
            ...
            trail = new MoonCreature.TrailNode[10];
            for (int index = 0; index < 10; ++index)
            {
                trail[index] = new MoonCreature.TrailNode()
                {
                    Position = Position,
        ...
        private void OnPlayer(Player player)
        {
            ...
            if ((double)vector2.LengthSquared() <= (double)bump.LengthSquared())
            {
                return;
            }

            bump = vector2;
            if ((double)(player.Center - start).Length() >= 200.0)
            {
                return;
            }
        ...
        {
            followingOffset = new Vector2(Calc.Random.Choose<int>(-1, 1) * Calc.Random.Range(8, 16), Calc.Random.Range(-20f, 0.0f));
        }
        ...
        public override void Update()
        {
            ...
            else
            {
                ...
                if ((double)(Position - start).Length() > 200.0 || followingTime <= 0.0)
                {
                    following = null;
                    targetTimer = 0.0f;
                }
            }
            speed += (target - Position).SafeNormalize() * (following == null ? 90f : 120f) * Engine.DeltaTime;
