...
namespace Celeste
{
    public class DreamWipe : ScreenWipe
    {
            ...
            : base(scene, wipeIn, onComplete)
        {
            ...
            DreamWipe.circles ??= new DreamWipe.Circle[(circleColumns + 2) * (circleRows + 2)];
            ...
            for (; num3 < circleColumns + 2; ++num3)
            {
                for (int index2 = 0; index2 < circleRows + 2; ++index2)
                {
                    ...
                    DreamWipe.circles[index1].Delay = Calc.Random.NextFloat(0.05f) + (float)((WipeIn ? circleColumns - num3 : num3) * 0.017999999225139618);
        ...
        public override void Update(Scene scene)
        {
            ...
            for (int index = 0; index < DreamWipe.circles.Length; ++index)
            {
                if (!WipeIn)
                {
                    ...
                    if (circles[index].Delay <= 0.0)
                    {
                        DreamWipe.circles[index].Radius += Engine.DeltaTime * 400f;
                    }
                }
                else if (circles[index].Radius > 0.0)
                {
                    DreamWipe.circles[index].Radius -= Engine.DeltaTime * 400f;
                }
                else
