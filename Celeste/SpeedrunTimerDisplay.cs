    ...
    public class SpeedrunTimerDisplay : Entity
    {
        ...
        public static void CalculateBaseSizes()
        {
            ...
            for (int index = 0; index < 10; ++index)
            {
                ...
                if (x > (double) SpeedrunTimerDisplay.numberWidth)
        ...
        public override void Render()
        {
            ...
            if (Settings.Instance.SpeedrunClock == SpeedrunType.Chapter)
            {
                ...
                SpeedrunTimerDisplay.DrawTime(new Vector2(x + 32f, Y + 44f), timeString, (float) (1.0 + wiggler.Value * 0.15000000596046448), session.StartedFromBeginning, scene.Completed, session.BeatBestTime);
            }
            else
            {
                ...
                string timeString1 = timeSpan1.TotalHours < 1.0 ? timeSpan1.ToString("mm\\:ss") : ((int) timeSpan1.TotalHours) + ":" + timeSpan1.ToString("mm\\:ss");
                TimeSpan timeSpan2 = TimeSpan.FromTicks(SaveData.Instance.Time);
                int totalHours = (int) timeSpan2.TotalHours;
                string timeString2 = totalHours + timeSpan2.ToString("\\:mm\\:ss\\.fff");
                ...
                SpeedrunTimerDisplay.DrawTime(new Vector2(x + 32f, (float) (Y + 40.0 + 26.400001525878906)), timeString1, (float) ((1.0 + wiggler.Value * 0.15000000596046448) * 0.60000002384185791), session.StartedFromBeginning, scene.Completed, session.BeatBestTime, 0.6f);
            }
        }
            ...
            float alpha = 1f)
        {
            ...
            for (int index = 0; index < timeString.Length; ++index)
            {
                ...
                Color color3 = ch == ':' || ch == '.' || num1 < (double) scale ? color2 : color1;
                float num2 = (float) ((ch == ':' || ch == '.' ? SpeedrunTimerDisplay.spacerWidth : (double) SpeedrunTimerDisplay.numberWidth) + 4.0) * num1;
