...
namespace Celeste
{
    ...
    public class WaterFall : Entity
    {
        ...
        public override void Awake(Scene scene)
        {
            ...
            bool flag = false;
            for (height = 8f; Y + (double) height < scene1.Bounds.Bottom && (water = Scene.CollideFirst<Water>(new Rectangle((int) X, (int) (Y + (double) height), 8, 8))) == null && ((solid = Scene.CollideFirst<Solid>(new Rectangle((int) X, (int) (Y + (double) height), 8, 8))) == null || !solid.BlockWaterfalls); solid = null)
                height += 8f;
            if (water != null && !Scene.CollideCheck<Solid>(new Rectangle((int) X, (int) (Y + (double) height), 8, 16)))
                flag = true;
        ...
        public override void Update()
        {
            ...
            {
                Vector2 position = new Vector2(X + 4f, (float) (Y + (double) height + 2.0));
                (Scene as Level).ParticlesFG.Emit(Water.P_Splash, 1, position, new Vector2(8f, 2f), new Vector2(0.0f, -1f).Angle());
            }
            base.Update();
        }

        public void RenderDisplacement() => Draw.Rect(X, Y, 8f, height, new Color(0.5f, 0.5f, 0.8f, 1f));
        ...
        public override void Render()
        {
            ...
            else
            {
                ...
                for (int index = 0; index < 6; ++index)
                    Draw.Rect((float) (X + (double) index + 1.0), Y, 1f, num - topSurface.GetSurfaceHeight(new Vector2(X + 1f + index, water.Y)), Water.FillColor);
