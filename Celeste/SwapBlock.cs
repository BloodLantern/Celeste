...
namespace Celeste
{
    ...
    public class SwapBlock : Solid
    {
        ...
        public SwapBlock(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Width, data.Height, data.Nodes[0] + offset, data.Enum<SwapBlock.Themes>("theme"))
        {
        }
        ...
        {
            base.Awake(scene);
            scene.Add(path = new SwapBlock.PathRenderer(this));
        }
        ...
        private void OnDash(Vector2 direction)
        {
            ...
            speed = lerp < 0.20000000298023224 ? MathHelper.Lerp(maxForwardSpeed * 0.333f, maxForwardSpeed, lerp / 0.2f) : maxForwardSpeed;
            Audio.Stop(returnSfx);
            Audio.Stop(moveSfx);
            if (!Swapping)
            {
                _ = Audio.Play("event:/game/05_mirror_temple/swapblock_move_end", Center);
            }
            else
            {
        ...
        public override void Update()
        {
            ...
            redAlpha = Calc.Approach(redAlpha, target == 1 ? 0.0f : 1f, Engine.DeltaTime * 32f);
            if (target == 0 && this.lerp == 0.0)
            {
            ...
            if (this.lerp != (double)lerp)
            {
                ...
                MoveTo(Vector2.Lerp(start, end, this.lerp), liftSpeed);
                ...
                if (position1 != position2)
                {
                    ...
                    if (Position == start && target == 0)
                    {
                        ...
                        _ = Audio.Play("event:/game/05_mirror_temple/swapblock_return_end", Center);
                    }
            ...
            StopPlayerRunIntoAnimation = (double)this.lerp is <= 0.0 or >= 1.0;
        }
        ...
        private class PathRenderer : Entity
        {
            ...
            public override void Render()
            {
                if (block.Theme != SwapBlock.Themes.Moon)
                {
                    for (int left = block.moveRect.Left; left < block.moveRect.Right; left += pathTexture.Width)
                    {
                        ...
                        {
                            _ = pathTexture.GetSubtexture(0, 0, Math.Min(pathTexture.Width, block.moveRect.Right - left), Math.Min(pathTexture.Height, block.moveRect.Bottom - top), clipTexture);
