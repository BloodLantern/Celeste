...
namespace Celeste
{
    public class CrumblePlatform : Solid
    {
        ...
        public override void Added(Scene scene)
        {
            ...
            outline = new List<Monocle.Image>();
            if ((double)Width <= 8.0)
            {
                Monocle.Image image = new(mtexture1.GetSubtexture(24, 0, 8, 8))
                {
            ...
            else
            {
                ...
                {
                    int num = x != 0 ? (x <= 0 || x >= (double)Width - 8.0 ? 2 : 1) : 0;
                    Monocle.Image image = new(mtexture1.GetSubtexture(num * 8, 0, 8, 8))
                    {
            ...
            {
                int num = (int)(((double)Math.Abs(X) + index) / 8.0) % 4;
                Monocle.Image image = new(mtexture2.GetSubtexture(num * 8, 0, 8, 8))
                {
                    Position = new Vector2(4 + index, 4f)
                };
        ...
        label_1:
            ...
            Input.Rumble(RumbleStrength.Medium, RumbleLength.Medium);
        label_7:
            _ = Audio.Play("event:/game/general/platform_disintegrate", crumblePlatform.Center);
            _ = crumblePlatform.shaker.ShakeFor(onTop ? 0.6f : 1f, false);
        ...
        private IEnumerator TileIn(int index, Monocle.Image img, float delay)
        {
            CrumblePlatform crumblePlatform = this;
            yield return delay;
            _ = Audio.Play("event:/game/general/platform_return", crumblePlatform.Center);
            img.Visible = true;
