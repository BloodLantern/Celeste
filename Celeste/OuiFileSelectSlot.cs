    ...
    public class OuiFileSelectSlot : Entity
    {
        ...
        public void CreateButtons()
        {
            ...
            if (Exists)
            {
                if (!Corrupted)
                {
                    ...
                    if (SaveData != null)
                    {
                        List<Button> buttons1 = buttons;
                        Button button1 = new Button();
                        button1.Label = Dialog.Clean("FILE_ASSIST_" + (AssistModeEnabled ? "ON" : "OFF"));
                        button1.Action = OnAssistSelected;
                        button1.Scale = 0.7f;
                        ...
                        {
                            List<Button> buttons2 = buttons;
                            Button button4 = new Button();
                            button4.Label = Dialog.Clean("FILE_VARIANT_" + (VariantModeEnabled ? "ON" : "OFF"));
                            button4.Action = OnVariantSelected;
                            button4.Scale = 0.7f;
            ...
            else
            {
                ...
                List<Button> buttons3 = buttons;
                Button button7 = new Button();
                button7.Label = Dialog.Clean("FILE_ASSIST_" + (AssistModeEnabled ? "ON" : "OFF"));
                button7.Action = OnAssistSelected;
                button7.Scale = 0.7f;
                ...
                if (!Settings.Instance.VariantsUnlocked)
                    return;
                List<Button> buttons4 = buttons;
                Button button10 = new Button();
                button10.Label = Dialog.Clean("FILE_VARIANT_" + (VariantModeEnabled ? "ON" : "OFF"));
                button10.Action = OnVariantSelected;
                button10.Scale = 0.7f;
        ...
        private void OnContinueSelected()
        {
            ...
            if (SaveData.Instance.CurrentSession != null && SaveData.Instance.CurrentSession.InArea)
            {
                ...
                FadeWipe fadeWipe = new FadeWipe(Scene, false, () => LevelEnter.Go(SaveData.Instance.CurrentSession, true));
            }
        ...
        private IEnumerator EnterFirstAreaRoutine()
        {
            ...
            yield return 0.3f;
            double num = overworld.Mountain.EaseCamera(0, AreaData.Areas[0].MountainZoom, 1f);
            yield return 0.4f;
        ...
        public Vector2 HiddenPosition(int x, int y) => !selected ? new Vector2(960f, Y) + new Vector2(x, y) * new Vector2(1920f, 1080f) : new Vector2(1920f, 1080f) / 2f + new Vector2(x, y) * new Vector2(1920f, 1080f);
        ...
        public override void Render()
        {
            ...
            if (selectedEase > 0.0)
            {
                ...
                for (int index = 0; index < buttons.Count; ++index)
                {
                    ...
                    vector2_1.Y += (float) (lineHeight * (double) button.Scale + 15.0);
                }
            }
            ...
            int num6 = 64;
            int num7 = 16;
            float num8 = (float) (Card.Width - (double) (num6 * 2) - 200.0) - num7;
            float x1 = (float) (-(double) Card.Width / 2.0 + num6 + 200.0 + num7 + num8 / 2.0);
            ...
            if (deletingEase > 0.0)
            {
                ...
                ActiveFont.Draw(Dialog.Clean("file_delete_really"), vector2 + new Vector2(0.0f, (float) (-16.0 - 64.0 * (1.0 - num11))), new Vector2(0.5f, 1f), Vector2.One, Color.White * num11);
                ActiveFont.DrawOutline(Dialog.Clean("file_delete_yes"), vector2 + new Vector2((float) ((!deleting || deleteIndex != 0 ? 0.0 : num2) * 1.2000000476837158) * num11, (float) (16.0 + 64.0 * (1.0 - num11))), new Vector2(0.5f, 0.0f), Vector2.One * 0.8f, deleting ? SelectionColor(deleteIndex == 0) : Color.Gray, 2f, Color.Black * num11);
                ActiveFont.DrawOutline(Dialog.Clean("file_delete_no"), vector2 + new Vector2((float) ((!deleting || deleteIndex != 1 ? 0.0 : num2) * 1.2000000476837158) * num11, (float) (16.0 + lineHeight + 64.0 * (1.0 - num11))), new Vector2(0.5f, 0.0f), Vector2.One * 0.8f, deleting ? SelectionColor(deleteIndex == 1) : Color.Gray, 2f, Color.Black * num11);
                if (failedToDeleteEase > 0.0)
                {
                    Vector2 position5 = new Vector2(960f, (float) (980.0 - 100.0 * deletingEase));
        ...
        public Color SelectionColor(bool selected)
        {
            if (!selected)
                return Color.White;
            return !Settings.Instance.DisableFlashes && !Scene.BetweenInterval(0.1f) ? TextMenu.HighlightColorB : TextMenu.HighlightColorA;
        }
