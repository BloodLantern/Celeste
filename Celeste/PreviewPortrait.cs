...
namespace Celeste
{
    public class PreviewPortrait : Scene
    {
        ...
        private readonly List<string> animations = new();
        private Vector2 topleft = new(64f, 64f);
        ...
        {
            return Mouse.X > (double)topleft.X && Mouse.Y > topleft.Y + (i * (double)ActiveFont.LineHeight) && (double)MInput.Mouse.X < 960.0 && Mouse.Y < topleft.Y + ((i + 1) * (double)ActiveFont.LineHeight);
        }
    }
}
