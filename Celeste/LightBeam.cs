...
namespace Celeste
{
    public class LightBeam : Entity
    {
        ...
        private Color color = new(0.8f, 1f, 1f);
        private float alpha;
        ...
        public override void Update()
        {
            ...
            if (entity != null && (string.IsNullOrEmpty(Flag) || scene.Session.GetFlag(Flag)))
            {
                ...
                float target = Math.Min(1f, (float)((double)Math.Max(0.0f, (float)((double)vector2_2.Length() - 8.0)) / LightLength));
                vector2_2 = vector2_1 - entity.Center;
                if ((double)vector2_2.Length() > LightWidth / 2.0)
                {
                    target = 1f;
                }
        ...
        public override void Render()
        {
            ...
            for (int index = 0; index < LightWidth; index += 4)
            {
                ...
                float offset = (float)Math.Sin((((double)num + (index * 32)) * 0.10000000149011612) + (Math.Sin(((double)num * 0.05000000074505806) + (index * 0.10000000149011612)) * 0.25)) * (float)((LightWidth / 2.0) - ((double)width / 2.0));
                float length = LightLength + ((float)Math.Sin((double)num * 0.25) * 8f);
