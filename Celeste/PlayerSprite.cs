...
namespace Celeste
{
    public class PlayerSprite : Sprite
    {
            ...
            : base(null, null)
        {
            ...
            spriteName = id;
            _ = GFX.SpriteBank.CreateOn(this, id);
        }

        public Vector2 HairOffset => Texture != null && PlayerSprite.FrameMetadata.TryGetValue(Texture.AtlasPath, out PlayerAnimMetadata playerAnimMetadata) ? playerAnimMetadata.HairOffset : Vector2.Zero;

        public float CarryYOffset => Texture != null && PlayerSprite.FrameMetadata.TryGetValue(Texture.AtlasPath, out PlayerAnimMetadata playerAnimMetadata) ? playerAnimMetadata.CarryYOffset * Scale.Y : 0.0f;

        public int HairFrame => Texture != null && PlayerSprite.FrameMetadata.TryGetValue(Texture.AtlasPath, out PlayerAnimMetadata playerAnimMetadata) ? playerAnimMetadata.Frame : 0;

        public bool HasHair => Texture != null && PlayerSprite.FrameMetadata.TryGetValue(Texture.AtlasPath, out PlayerAnimMetadata playerAnimMetadata) && playerAnimMetadata.HasHair;

        public bool Running => LastAnimationID != null && (LastAnimationID == "flip" || LastAnimationID.StartsWith("run"));
