...
namespace Celeste
{
    public class WaveDashPresentation : Entity
    {
        ...
        private readonly List<WaveDashPage> pages = new();
        private float pageEase;
        private bool pageTurning;
        private bool pageUpdating;
        private bool waitingForPageTurn;
        private readonly VertexPositionColorTexture[] verts = new VertexPositionColorTexture[6];
        private readonly EventInstance usingSfx;
        ...
        public WaveDashPresentation(EventInstance usingSfx = null)
        {
            ...
            RunThread.Start(new Action(LoadingThread), "Wave Dash Presentation Loading", true);
        }
        ...
        private IEnumerator Routine()
        {
            ...
            presentation.Add(new BeforeRenderHook(new Action(presentation.BeforeRender)));
            ...
            while (presentation.pageIndex < presentation.pages.Count)
            {
                ...
                if (presentation.pageIndex < presentation.pages.Count)
                {
                    ...
                    yield return (float)((double)duration * 0.800000011920929);
                }
            }
            ...
            _ = Audio.Play("event:/new_content/game/10_farewell/cafe_computer_off");
        ...
        {
            if (CurrPage.Transition is not WaveDashPage.Transitions.ScaleIn and not WaveDashPage.Transitions.FadeIn)
            {
                if (CurrPage.Transition == WaveDashPage.Transitions.Rotate3D)
                {
                    _ = Audio.Play("event:/new_content/game/10_farewell/ppt_cube_transition");
                }
                else if (CurrPage.Transition == WaveDashPage.Transitions.Blocky)
                {
                    _ = Audio.Play("event:/new_content/game/10_farewell/ppt_dissolve_transition");
                }
                else if (CurrPage.Transition == WaveDashPage.Transitions.Spiral)
                {
                    _ = Audio.Play("event:/new_content/game/10_farewell/ppt_spinning_transition");
                }
            }
            while (pageEase < 1.0)
            {
        ...
        private void BeforeRender()
        {
            ...
            {
                Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D)prevPageBuffer);
            ...
            if (CurrPage != null)
            {
                Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D)currPageBuffer);
                Engine.Graphics.GraphicsDevice.Clear(CurrPage.ClearColor);
                Draw.SpriteBatch.Begin();
                CurrPage.Render();
                Draw.SpriteBatch.End();
            }
            Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D)screenBuffer);
            ...
            if (pageTurning)
            {
                if (CurrPage.Transition == WaveDashPage.Transitions.ScaleIn)
                {
                    ...
                    Draw.SpriteBatch.Draw((RenderTarget2D)currPageBuffer, ScaleInPoint, new Rectangle?(currPageBuffer.Bounds), Color.White, 0.0f, ScaleInPoint, Vector2.One * pageEase, SpriteEffects.None, 0.0f);
                    Draw.SpriteBatch.End();
                }
                ...
                else if (CurrPage.Transition == WaveDashPage.Transitions.Blocky)
                {
                    ...
                    for (int x = 0; x < ScreenWidth; x += num)
                    {
                        ...
                        {
                            if ((double)WaveDashPresentation.PseudoRandRange(ref seed, 0.0f, 1f) <= pageEase)
                            {
                                Draw.SpriteBatch.Draw((RenderTarget2D)currPageBuffer, new Rectangle(x, y, num, num), new Rectangle?(new Rectangle(x, y, num, num)), Color.White);
                            }
                        }
                    }
                    Draw.SpriteBatch.End();
                ...
                else
                {
                    ...
                    Draw.SpriteBatch.Begin();
                    Draw.SpriteBatch.Draw((RenderTarget2D)prevPageBuffer, Vector2.Zero, Color.White);
                    Draw.SpriteBatch.Draw((RenderTarget2D)currPageBuffer, Celeste.TargetCenter, new Rectangle?(currPageBuffer.Bounds), Color.White, (float)((1.0 - pageEase) * 12.0), Celeste.TargetCenter, Vector2.One * pageEase, SpriteEffects.None, 0.0f);
                    Draw.SpriteBatch.End();
                }
            }
            else
        ...
        private void RenderQuad(Texture texture, float ease, float rotation)
        {
            ...
            float x = num1;
            float y = 1f;
            Vector3 vector3_1 = new(-x, y, 0.0f);
            Vector3 vector3_2 = new(x, y, 0.0f);
            Vector3 vector3_3 = new(x, -y, 0.0f);
            Vector3 vector3_4 = new(-x, -y, 0.0f);
        ...
        public override void Render()
        {
            ...
            Rectangle rectangle = new((int)((1920.0 - (double)width) / 2.0), (int)((1080.0 - (double)height) / 2.0), (int)width, (int)height);
        ...
        {
            return min + (float)(WaveDashPresentation.PseudoRand(ref seed) % 1000U / 1000.0 * ((double)max - (double)min));
        }
    }
}
