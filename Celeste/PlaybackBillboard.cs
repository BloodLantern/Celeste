...
namespace Celeste
{
    ...
    public class PlaybackBillboard : Entity
    {
        ...
        public override void Awake(Scene scene)
        {
            ...
            int x1 = (int) (Width / 8.0);
            int y1 = (int) (Height / 8.0);
        ...
        private bool Empty(int x, int y) => !Scene.CollideCheck<PlaybackBillboard>(new Rectangle((int) X + x * 8, (int) Y + y * 8, 8, 8));
        ...
        private void RenderBloom() => Draw.Rect(Collider, Color.White * 0.4f);
        ...
        public static void DrawNoise(Rectangle bounds, ref uint seed, Color color)
        {
            ...
            Vector2 vector2_1 = new Vector2(PlaybackBillboard.PseudoRandRange(ref seed, 0.0f, mtexture.Width / 2), PlaybackBillboard.PseudoRandRange(ref seed, 0.0f, mtexture.Height / 2));
            Vector2 vector2_2 = new Vector2(mtexture.Width, mtexture.Height) / 2f;
            for (float num1 = 0.0f; num1 < (double) bounds.Width; num1 += vector2_2.X)
            {
                float width = Math.Min(bounds.Width - num1, vector2_2.X);
                for (float num2 = 0.0f; num2 < (double) bounds.Height; num2 += vector2_2.Y)
                {
                    float height = Math.Min(bounds.Height - num2, vector2_2.Y);
                    Rectangle rectangle = new Rectangle((int) (mtexture.ClipRect.X + (double) vector2_1.X), (int) (mtexture.ClipRect.Y + (double) vector2_1.Y), (int) width, (int) height);
                    Draw.SpriteBatch.Draw(mtexture.Texture.Texture, new Vector2(bounds.X + num1, bounds.Y + num2), rectangle, color);
                }
            }
        }
        ...
        private static float PseudoRandRange(ref uint seed, float min, float max) => min + (float) (PlaybackBillboard.PseudoRand(ref seed) % 1000U / 1000.0 * (max - (double) min));
