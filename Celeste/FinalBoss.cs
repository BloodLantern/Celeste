    ...
    public class FinalBoss : Entity
    {
        ...
        public override void Added(Scene scene)
        {
            ...
            if (patternIndex == 0 && !level.Session.GetFlag("boss_intro") && level.Session.Level.Equals("boss-00"))
            {
                ...
                Alarm.Set(this, 0.5f, () => OnPlayer(null));
            light.Position = (Sprite != null ? Sprite : (GraphicsComponent) NormalSprite).Position + new Vector2(0.0f, -10f);
        }
        ...
        public override void Awake(Scene scene)
        {
            ...
            fallingBlocks.Sort((a, b) => (int) (a.X - (double) b.X));
            movingBlocks = Scene.Tracker.GetEntitiesCopy<FinalBossMovingBlock>();
            movingBlocks.Sort((a, b) => (int) (a.X - (double) b.X));
        }
        ...
        public override void Update()
        {
            ...
            if (!Sitting)
            {
                ...
                {
                    if (facing == -1 && entity.X > X + 20.0)
                    {
                        facing = 1;
                        scaleWiggler.Start();
                    }
                    else if (facing == 1 && entity.X < X - 20.0)
                    {
                        facing = -1;
                        scaleWiggler.Start();
                    }
                ...
                if (!Moving && entity != null)
                {
                    ...
                    target = length > 0.0 ? (Center - entity.Center).SafeNormalize(length) : Vector2.Zero;
                }
                else
                    target = Vector2.Zero;
        ...
        public override void Render()
        {
            if (Sprite != null)
            {
                ...
                sprite.Scale *= (float) (1.0 + scaleWiggler.Value * 0.20000000298023224);
            }
            if (NormalSprite != null)
            {
        ...
        private void TriggerFallingBlocks(float leftOfX)
        {
            ...
            int num = 0;
            while (fallingBlocks.Count > 0 && fallingBlocks[0].X < (double) leftOfX)
            {
