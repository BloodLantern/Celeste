...
namespace Celeste
{
    ...
    public class Water : Entity
    {
            ...
            float height)
        {
            ...
            grid = new bool[(int) (width / 8.0), (int) (height / 8.0)];
            fill = new Rectangle(0, 0, (int) width, (int) height);
            int y = 8;
            if (topSurface)
            {
                TopSurface = new Surface(Position + new Vector2(width / 2f, y), new Vector2(0.0f, -1f), width, height);
            ...
            if (bottomSurface)
            {
                BottomSurface = new Surface(Position + new Vector2(width / 2f, height - y), new Vector2(0.0f, 1f), width, height);
        ...
        public class Surface
        {
            ...
            public Surface(Vector2 position, Vector2 outwards, float width, float bodyHeight)
            {
                ...
                int num2 = (int) (width * 0.20000000298023224);
                Rays = new List<Ray>();
            ...
            public void RemoveTension(Tension tension) => Tensions.Remove(tension);
            ...
            public float GetSurfaceHeight(Vector2 position) => GetSurfaceHeight(GetPointAlong(position));

            public float GetSurfaceHeight(float position)
            {
                if (position < 0.0 || position > (double) Width)
                    return 0.0f;
                float num1 = 0.0f;
                foreach (Ripple ripple in Ripples)
                {
                    float val = Math.Abs(ripple.Position - position);
                    float num2 = val >= 12.0 ? Calc.ClampedMap(val, 16f, 32f, -0.75f, 0.0f) : Calc.ClampedMap(val, 0.0f, 16f, 1f, -0.75f);
                ...
                foreach (Tension tension in Tensions)
                {
                    ...
                    num3 += (float) (Ease.CubeOut(t) * (double) tension.Strength * 12.0);
                }
                float val1 = position / Width;
                return num3 * Calc.ClampedMap(val1, 0.0f, 0.1f, 0.5f) * Calc.ClampedMap(val1, 0.9f, 1f, 1f, 0.5f) + (float) Math.Sin(timer + position * 0.10000000149011612) + 6f;
            }

            public void Render(Camera camera) => GFX.DrawVertices(camera.Matrix, mesh, mesh.Length);
        }
    }
}
