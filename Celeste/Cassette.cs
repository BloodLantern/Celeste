...
namespace Celeste
{
    public class Cassette : Entity
    {
        ...
        public override void Added(Scene scene)
        {
            ...
            sprite.Play("idle");
            Add(scaleWiggler = Wiggler.Create(0.25f, 4f, f => sprite.Scale = Vector2.One * (float) (1.0 + f * 0.25)));
        ...
        public override void Update()
        {
            ...
            if (collecting || !Scene.OnInterval(0.1f))
                return;
            SceneAs<Level>().Particles.Emit(Cassette.P_Shine, 1, Center, new Vector2(12f, 10f));
        }
        ...
        private IEnumerator CollectRoutine(Player player)
        {
            ...
            level.Session.RespawnPoint = level.GetSpawnPoint(cassette.nodes[1]);
            ...
            float p;
            for (p = 0.0f; p < 1.5; p += Engine.DeltaTime)
            {
            ...
            float duration = 0.4f;
            for (p = 0.0f; p < 1.0; p += Engine.DeltaTime / duration)
            {
        ...
        private class UnlockedBSide : Entity
        {
            ...
            public IEnumerator EaseIn()
            {
                ...
                while ((unlockedBside.alpha += Engine.DeltaTime / 0.5f) < 1.0)
                    yield return null;
            ...
            public IEnumerator EaseOut()
            {
                ...
                while ((unlockedBside.alpha -= Engine.DeltaTime / 0.5f) > 0.0)
                    yield return null;
            ...
            public override void Render()
            {
                ...
                ActiveFont.Draw(text, vector2_1 + vector2_2, new Vector2(0.5f, 0.0f), Vector2.One, Color.White * num);
                if (!waitForKeyPress)
                    return;
                GFX.Gui["textboxbutton"].DrawCentered(new Vector2(1824f, 984 + (timer % 1.0 < 0.25 ? 6 : 0)));
            }
        }
    }
}
