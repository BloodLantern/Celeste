...
namespace Monocle
{
    public class Scene : IEnumerable<Entity>, IEnumerable
    {
        ...
        {
            return (int)((TimeActive - (double)Engine.DeltaTime) / (double)interval) < (int)(TimeActive / (double)interval);
        }

        public bool OnInterval(float interval, float offset)
        {
            return Math.Floor((TimeActive - (double)offset - (double)Engine.DeltaTime) / (double)interval) < Math.Floor((TimeActive - (double)offset) / (double)interval);
        }
        ...
        {
            return (int)((RawTimeActive - (double)Engine.RawDeltaTime) / (double)interval) < (int)(RawTimeActive / (double)interval);
        }

        public bool OnRawInterval(float interval, float offset)
        {
            return Math.Floor((RawTimeActive - (double)offset - (double)Engine.RawDeltaTime) / (double)interval) < Math.Floor((RawTimeActive - (double)offset) / (double)interval);
        }
