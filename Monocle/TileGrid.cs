...
namespace Monocle
{
    public class TileGrid : Component
    {
        ...
        {
            for (int index1 = 0; index1 < tiles.GetLength(0) && index1 + offsetX < TilesX; ++index1)
            {
                for (int index2 = 0; index2 < tiles.GetLength(1) && index2 + offsetY < TilesY; ++index2)
                {
                    Tiles[index1 + offsetX, index2 + offsetY] = tileset[tiles[index1, index2]];
                }
            }
        }
        ...
        public Rectangle GetClippedRenderTiles()
        {
            ...
            else
            {
                ...
                val1_1 = (int)Math.Max(0.0, Math.Floor(((double)clipCamera.Left - vector2.X) / TileWidth) - VisualExtend);
                val1_2 = (int)Math.Max(0.0, Math.Floor(((double)clipCamera.Top - vector2.Y) / TileHeight) - VisualExtend);
