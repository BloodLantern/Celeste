...
namespace Monocle
{
    public static class Draw
    {
        ...
        {
            Draw.HollowRect(rect.X, rect.Y, rect.Width, rect.Height, color);
        }
            ...
            float scale)
        {
            ...
            for (int x = -1; x < 2; ++x)
            {
                for (int y = -1; y < 2; ++y)
                {
                    if (x != 0 || y != 0)
                    {
                        Draw.SpriteBatch.DrawString(font, text, position.Floor() + new Vector2(x, y), Color.Black, 0.0f, origin, scale, SpriteEffects.None, 0.0f);
                    }
                }
            }
            Draw.SpriteBatch.DrawString(font, text, position.Floor(), color, 0.0f, origin, scale, SpriteEffects.None, 0.0f);
            ...
            Color outlineColor)
        {
            ...
            for (int x = -1; x < 2; ++x)
            {
                for (int y = -1; y < 2; ++y)
                {
                    if (x != 0 || y != 0)
                    {
                        Draw.SpriteBatch.DrawString(font, text, position.Floor() + new Vector2(x, y), outlineColor, 0.0f, origin, 1f, SpriteEffects.None, 0.0f);
                    }
                }
            }
            Draw.SpriteBatch.DrawString(font, text, position.Floor(), color, 0.0f, origin, 1f, SpriteEffects.None, 0.0f);
            ...
            float scale)
        {
            ...
            for (int x = -1; x < 2; ++x)
            {
                for (int y = -1; y < 2; ++y)
                {
                    if (x != 0 || y != 0)
                    {
                        Draw.SpriteBatch.DrawString(font, text, position.Floor() + new Vector2(x, y), outlineColor, 0.0f, origin, scale, SpriteEffects.None, 0.0f);
                    }
                }
            }
            Draw.SpriteBatch.DrawString(font, text, position.Floor(), color, 0.0f, origin, scale, SpriteEffects.None, 0.0f);
