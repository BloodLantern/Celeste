    ...
    public struct Particle
    {
        ...
        public void Update(float? delta = null)
        {
            ...
            else
            {
                ...
                float num2 = Type.FadeMode != ParticleType.FadeModes.Linear ? (Type.FadeMode != ParticleType.FadeModes.Late ? (Type.FadeMode != ParticleType.FadeModes.InAndOut ? 1f : ((double)num1 <= 0.75 ? ((double)num1 >= 0.25 ? 1f : num1 / 0.25f) : (float)(1.0 - (((double)num1 - 0.75) / 0.25)))) : Math.Min(1f, num1 / 0.25f)) : num1;
                if ((double)num2 == 0.0)
                {
                ...
                {
                    Speed *= (float)Math.Pow(Type.SpeedMultiplier, (double)y);
                }
        ...
        public void Render()
        {
            Vector2 position = new((int)Position.X, (int)Position.Y);
            if (Track != null)
            {
                position += Track.Position;
            }

            Draw.SpriteBatch.Draw(Source.Texture.Texture, position, new Rectangle?(Source.ClipRect), Color, Rotation, Source.Center, Size, SpriteEffects.None, 0.0f);
        }
        ...
        public void Render(float alpha)
        {
            ...
            Draw.SpriteBatch.Draw(Source.Texture.Texture, position, new Rectangle?(Source.ClipRect), Color * alpha, Rotation, Source.Center, Size, SpriteEffects.None, 0.0f);
        }
    }
}
